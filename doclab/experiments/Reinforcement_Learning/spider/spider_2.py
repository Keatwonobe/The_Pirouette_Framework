...
Testing the measurement algorithm on real Gym Pendulum-v1
...
# --- NEW: use a real Pendulum implementation via gym/gymnasium ---
...
# ------------------------------------------------------------------
#  Active Manifold Components (unchanged logic, reused as-is)
# ------------------------------------------------------------------
...
class TriangulationNetwork:
    ...
    def forward(self, state):
        ...
        Returns:
            tension:   (batch, action_dim)
            phase:     (batch, action_dim)
            coherence: (batch, 1)
        """
...
class CoherenceMemory:
    ...
    def sample(self, batch_size):
        """Sample experiences weighted by coherence (currently uniform sample)"""
...
class SpiderAgent:
    ...
    def __init__(self, state_dim, action_dim, lr=1e-3):
        ...
        # Probe weights (learned attention policy over discrete actions)
    ...
    def select_action(self, state, epsilon=0.1):
        """Select action via active probing protocol (discrete index)"""
    ...
    def triangulate(self, state):
        ...
        # Position = weighted combination (one position per action)
    ...
    def update(self, batch_size=64, gamma=0.99):
        ...
        self._backward(states, actions, q_current, targets,
                       coherences, coherence_pred[:, 0],
                       coherence_target, cache)
        
        return total_loss
    
    def _backward(self, states, actions, q_current, targets,
                  coherences, coherence_pred, coherence_target, cache):
        """Manual backpropagation (tension + coherence heads only)"""
...
# ------------------------------------------------------------------
#  Training loop on real Pendulum-v1
# ------------------------------------------------------------------

def make_pendulum_env():
    """
    Create a real Pendulum-v1 environment.

    Works with both gymnasium and gym, depending on what's installed.
    """
    # gymnasium uses 'Pendulum-v1', recent gym does too.
    env = gym.make("Pendulum-v1")
    return env


def reset_env(env):
    """Handle both gymnasium and gym reset signatures."""
    out = env.reset()
...
def step_env(env, action):
    """Handle both gymnasium and gym step signatures."""
    out = env.step(action)
    if len(out) == 5:
...
def train_spider_agent(episodes=1500,
                       max_steps=200,
                       action_dim=5,
                       action_low=-2.0,
                       action_high=2.0):
    """
    Train Spider Protocol agent on real Pendulum-v1.
    ...
    state = reset_env(env)
    state_dim = state.shape[0]  # should be 3 for Pendulum-v1

    agent = SpiderAgent(state_dim=state_dim, action_dim=action_dim)

    # Discretize action space in [-2, 2]
    action_space = np.linspace(action_low, action_high, action_dim)
    ...
    for episode in range(episodes):
        state = reset_env(env)
        episode_reward = 0.0
        epsilon = max(0.01, 0.5 - episode / 150.0)

        step = 0

        while step < max_steps:
            # Select discrete action index via active probing
            action_idx = agent.select_action(state, epsilon)
            action_value = action_space[action_idx]
            # Gym pendulum expects shape (1,) np.array
            ...
            # expected_next is length action_dim; compare first state_dim entries
        ...
        if episode % 20 == 0:
            ...
            print(f"Episode {episode:3d} | Avg Reward: {avg_reward:7.2f} | "
                  f"Epsilon: {epsilon:.3f}")
            print(f"  Probe weights: "
                  f"{' '.join([f'{w:.3f}' for w in agent.probe_weights])}")
            if episode > 0:
                utilities = agent.probe_utilities / (agent.probe_counts + 1e-8)
                print(f"  Probe utilities: "
                      f"{' '.join([f'{u:.2f}' for u in utilities])}")
...
if __name__ == "__main__":
    ...
    agent, rewards = train_spider_agent(episodes=1500)
    ...
    # For inspection / scoring
    action_dim = len(agent.probe_weights)
    action_space = np.linspace(-2.0, 2.0, action_dim)
    print("Final probe weight distribution:")
    for i, (w, a) in enumerate(zip(agent.probe_weights, action_space)):
        print(f"  Action {i} (value={a:+.2f}): weight={w:.4f}")

    print()
    print("Probe utility scores (avg reward per probe):")
    utilities = agent.probe_utilities / (agent.probe_counts + 1e-8)
    for i, (u, a, c) in enumerate(zip(utilities, action_space, agent.probe_counts)):
        print(f"  Action {i} (value={a:+.2f}): "
              f"utility={u:.3f}, count={int(c)}")
