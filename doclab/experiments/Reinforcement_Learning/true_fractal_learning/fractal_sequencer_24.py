...
class ReflexiveBallast:
    ...
    def get_stabilization(self, hull_angle, velocity, knottedness):
        ...
        # "Brace for Chaos"
        # We cap the brace to prevent infinite stiffness
...
# ============================================================
# 4. CHIMERA AGENT v24 (The Ratchet)
# ============================================================
class ChimeraAgent:
...
# ============================================================
# 5. RATCHET TRAINER (The Fix)
# ============================================================
class RatchetTrainer:
    def __init__(self, env_name):
        self.env_name = env_name
        # Faster Cycle: 2 Inhale, 2 Exhale
        self.breath_cycle = 4 
    ...
    def evaluate(self, gene, phase, render=False):
        ...
        if phase == "INHALE":
            ...
            # You only get the knot bonus if you traveled!
            # This prevents "seizures at x=0"
            # Logarithmic scaling prevents knots from dominating distance.
            knot_bonus = math.log1p(total_knot) 
            fitness = dist * (1.0 + (knot_bonus * 0.1))
            
        else: # EXHALE
            # PURE EFFICIENCY
            # We don't punish knots. We just ignore them.
            # If the knots help you walk further, you win.
            # If they don't, you lose to someone who walked simpler/further.
            fitness = dist 
            
        return fitness, dist, total_knot

    def run(self):
        print("🔧 RATCHET PROTOCOL v24")
        print("   Inhale: Dist * Complexity. Exhale: Pure Dist.")
        ...
        for g in range(100):
            ...
            print(f"   ★ Best: {gen_best_dist:.1f}m (Knot: {gen_best_knot:.0f}) | Fit: {gen_best_fit:.1f}")
            
            # Elitist Mutation
            population = [best_gene.copy()] 
            for _ in range(11):
...
if __name__ == "__main__":
    trainer = RatchetTrainer("BipedalWalker-v3")
    best = trainer.run()
    print("\nVerifying Ratchet...")
