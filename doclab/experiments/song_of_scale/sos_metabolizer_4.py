...
# -*- coding: utf-8 -*-
"""
SoS Metabolizer (v2.3, market-style quadrants)
- Hardened I/O (NaN/Inf guard, empty-band safe)
- Bandwise relabeling with configurable quantile thresholds
- Four-projection planes, allocation bars, triads plot
- NEW: Market-style "phase-space" quadrant plot per band
"""
import argparse
import json
...
# ---------- Market-style phase-space (one per band) ----------
def plot_market_style(dfw_lab: pd.DataFrame, thresholds: dict, outdir: Path, max_points=120000):
    if dfw_lab.empty:
        return
    ensure_dir(outdir / "plots")
    for (blo,bhi), sub in dfw_lab.groupby(["band_lo","band_hi"]):
        ...
        k_cut = th.get("th_k_low", np.nan)
        dp_cut = th.get("th_dP", np.nan)

        fig = plt.figure(figsize=(9,7))
        # scatter all (group by state for legend)
        for s, chunk in d.groupby("state"):
            plt.scatter(chunk["kappa_mag"], chunk["deltaP"], s=8, alpha=0.6, label=str(s))
        # quadrant lines (dashed)
        if np.isfinite(k_cut):
            plt.axvline(k_cut, linestyle="--", linewidth=1)
        if np.isfinite(dp_cut):
            plt.axhline(dp_cut, linestyle="--", linewidth=1)
        plt.axhline(0.0, linestyle=":", linewidth=1)
        plt.xlim(x_min, x_max); plt.ylim(y_min, y_max)
        plt.xlabel("Intrinsic Chirality |κ*|  → Complexity/Stress")
        plt.ylabel("Performance ΔP (relative) → Value Creation")
        plt.title(f"Phase-Space (Market-Style)  [{blo:.3g}–{bhi:.3g} Hz]")
        # annotate quadrants (I–IV)
        # positions use midpoints padded inside the axes
        xm = 0.5*(x_min + x_max); ym = 0.5*(y_min + y_max)
        if np.isfinite(k_cut) and np.isfinite(dp_cut):
            xL, xR = (x_min + k_cut)/2, (k_cut + x_max)/2
            yB, yT = (y_min + dp_cut)/2, (dp_cut + y_max)/2
        else:
            xL, xR = (x_min + xm)/2, (xm + x_max)/2
            yB, yT = (y_min + ym)/2, (ym + y_max)/2
        # I: Weaver (low k, high ΔP)
        plt.text(xL, yT, "I. Weaver\n(Low κ, High ΔP)", ha="center", va="center")
        # II: Gladiator (high k, high ΔP)
        plt.text(xR, yT, "II. Gladiator\n(High κ, High ΔP)", ha="center", va="center")
        # III: Drifter (low k, low ΔP)
        plt.text(xL, yB, "III. Drifter\n(Low κ, Low ΔP)", ha="center", va="center")
        # IV: Vortex (high k, low ΔP)
        plt.text(xR, yB, "IV. Vortex\n(High κ, Low ΔP)", ha="center", va="center")
        plt.legend(markerscale=1.6, fontsize=9)
        fig.tight_layout()
        fig.savefig(outdir / "plots" / f"phase_space_band_{blo:.6g}-{bhi:.6g}.png", dpi=170)
        plt.close(fig)
...
def main():
    ap = argparse.ArgumentParser(description="Metabolize SoS outputs into plots & summaries (v2.3 market-style)")
    ...
    ap.add_argument("--max-points", type=int, default=120000, help="Max points per scatter to keep plots fast")
    # Threshold controls
    ...
    plot_market_style(dfw_lab, thresholds, outdir, max_points=args.max_points)
