...
def hydrate_character_from_dict(data, filename=None):
    ...
    - If explicit pools are present, we respect them.
    - If not, but we have pools_profile + level, we derive HP/ENT from that.
    - Otherwise, we fall back to simple defaults (HP=10, ENT=0).
    """
    ...
    pp = data.get("pools_profile", {}) or {}

    def derive_pool(total_key: str, per_key: str, default_per: int):
        """
        If total_key not already in pools, derive from pools_profile[per_key] * level.

        pools_profile[per_key] may be:
          - a single number
          - a [min, max] list, in which case we use the average
        """
        if total_key in pools:
            return
        per_val = None
        per = pp.get(per_key)
        if isinstance(per, (int, float)):
            ...
            # use the average across the range for a stable but “on theme” value
            try:
                nums = [float(x) for x in per]
                per_val = sum(nums) / len(nums)
            except Exception:
                per_val = None
        if per_val is None:
            per_val = float(default_per)
        pools[total_key] = int(per_val * max(level, 1))

    # Derive HP/ENT if not already set
    derive_pool("HP",  "HP_per_level", 10)
    derive_pool("ENT", "ENT_per_level", 0)

    # AEP still defaults to 0 for now
    pools.setdefault("HP", 10)
    pools.setdefault("ENT", 0)
    pools.setdefault("AEP", 0)
...
def print_state(characters):
    ...
    print("  side IDX[..] LABEL    change side/team at runtime")
    print("  look")
    print("==============\n")
...
def handle_player_turn(player, characters, libs):
    while True:
        ...
        # side toggles: side 3-5 guards / side 10-12 cultists / etc.
        if op == "side":
            if len(parts) < 3:
                print("usage: side IDX[..] LABEL")
                continue
            # last token is the new side label; everything before are indices
            new_side = parts[-1]
            idx_tokens = parts[1:-1]
            indices = parse_index_tokens(idx_tokens, len(characters))
            if not indices:
...
def main():
    ...
    # === PREFLIGHT STAGING ===
    preflight_setup(chars)

    # track whether this scene *ever* had players at the start of combat
    ...
    for rnd in range(1, max_rounds + 1):
        ...
        # Only auto-end if this was actually a player-involved scene.
