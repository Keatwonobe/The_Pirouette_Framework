...
class Config:
    ...
    # --- PRG Hyperparameters ---
    prg_window_size: int = 128  # How far back to look for PRG calculations
    wK: float = 0.1             # Weight for Coherence gain
    wG: float = 0.05            # Weight for Pressure cost
    wBeat: float = 0.01           # Weight for Beat consistency penalty
    zeta_G: float = 0.1
    zeta_K: float = 0.1
...
def train():
    ...
    for epoch in range(cfg.epochs):
        ...
        for t in range(cfg.steps_per_epoch):
            ...
            if (t + 1) % cfg.prg_window_size == 0:
                # --- Calculate PRG rewards ---
                with torch.no_grad(): # Ensure PRG calculations don't affect gradients
                ...
                rew_buf[t] += shaped_reward # Add shaped reward
            ...
            if terminal or epoch_ended:
                ...
                # --- GAE Calculation ---
                last_val = v
                delta_buf = torch.zeros(cfg.steps_per_epoch, device=device)
                for i in reversed(range(t + 1)):
                    next_val = last_val if i == t else val_buf[i+1]
                    delta_buf[i] = rew_buf[i] + cfg.gamma * next_val - val_buf[i]

                last_adv = 0
                for i in reversed(range(t + 1)):
                    adv_buf[i] = delta_buf[i] + cfg.gamma * cfg.lam * last_adv
                    last_adv = adv_buf[i]
                ...
                o, _ = env.reset()
                ep_ret, ep_len = 0, 0
                # We break here to simplify, as the buffer is now full for this epoch
                break 
