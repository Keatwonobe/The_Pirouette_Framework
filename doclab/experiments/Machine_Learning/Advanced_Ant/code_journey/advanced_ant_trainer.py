...
class ContinuousPirouetteAnalytics:
    ...
    def temporal_coherence_spectrum(self, data, scales=None, wavelet='cmor1.0-1.0'):
        ...
        if scales is None:
            scales = np.arange(1, min(len(data) // 4, 64))
...
class ContinuousShadowRegionDetector:
    ...
    def __init__(self, analytics, shadow_threshold=0.01, min_shadow_width=3):
        self.analytics = analytics
        self.shadow_threshold = shadow_threshold
        self.min_shadow_width = min_shadow_width
    ...
    def analyze_and_suggest(self, data):
        ...
        hyperparameters to escape them.
        """
        ...
        _, scales, coherence, = self.analytics.temporal_coherence_spectrum(data_norm)
        if coherence is None:
            return {}
            
        # Detect shadow regions (low coherence)
        is_in_shadow = coherence < self.shadow_threshold
        
        # Simple suggestion logic based on the presence and depth of shadows
        suggestions = {}
        mean_coherence = np.mean(coherence)

        if np.any(is_in_shadow) and mean_coherence < 0.1: # Deep, significant shadows
            print("INFO: Deep shadows detected. Suggesting aggressive changes.")
            suggestions['critic_lr'] = 0.001 # Increase LR to escape
            suggestions['actor_lr'] = 0.0005
            suggestions['batch_size'] = 512 # Larger batch to stabilize gradients
            suggestions['alpha'] = 0.1 # Lower alpha for more exploitation
            
        elif np.any(is_in_shadow): # Moderate shadows
            print("INFO: Moderate shadows detected. Suggesting Ki-based changes.")
            suggestions['critic_lr'] = self.ki_rest / 10000 # ~0.0004
            suggestions['actor_lr'] = self.ki_rest / 15000 # ~0.00027
            suggestions['batch_size'] = 256
            suggestions['alpha'] = 0.2 # Standard alpha
...
class SACAgent:
    ...
    def __init__(self, env, hidden_dim=256, lr=3e-4, gamma=0.99, tau=0.005, alpha=0.2):
        ...
        self.td_errors = [] # For shadow analysis
    ...
    def update(self, batch_size):
        ...
        # Record TD error for shadow analysis
...
class ShadowAwareTrainer:
    ...
    def train(self):
        ...
        for episode in range(self.num_episodes):
            ...
            print(f"Episode: {episode+1}, Total Steps: {total_steps}, Reward: {episode_reward:.2f}, Avg Reward: {avg_reward:.2f}")
            ...
            if episode > 0 and (episode + 1) % self.analysis_freq == 0:
                ...
                # Analyze the last 2000 TD errors for stability
                suggestions = self.shadow_detector.analyze_and_suggest(
                    np.array(self.agent.td_errors[-2000:])
                )
                
                # Apply suggestions to the agent and trainer
                self.agent.apply_suggestions(suggestions)
                if 'batch_size' in suggestions:
                    self.batch_size = suggestions['batch_size']
                    print(f"Updating Trainer Batch Size to: {self.batch_size}")
                print("-------------------------------------------\n")
    ...
    def plot_rewards(self):
        ...
        if len(self.reward_history) >= 100:
            moving_avg = np.convolve(self.reward_history, np.ones(100)/100, mode='valid')
            plt.plot(np.arange(99, len(self.reward_history)), moving_avg, 'r', label='100-episode MA')
        ...
        print("Training plot saved to 'shadow_aware_training_rewards.png'")
