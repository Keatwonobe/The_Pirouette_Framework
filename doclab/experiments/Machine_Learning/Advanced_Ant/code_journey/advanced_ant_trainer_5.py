...
# (Add this to the UTILS section)
def calculate_trajectory_fingerprint(state_history):
    """
    Calculates a fractal fingerprint for an episode's state trajectory.
    """
    if len(state_history) < 50:
        return {'hfd_xy': 1.0} # Default fingerprint for short episodes

    # Extracting the x,y position of the ant's torso from the state
    # state[0] is x-coord, state[1] is y-coord
    xy_coords = np.array([state[0:2] for state in state_history])
    
    # Calculate a single series from the 2D path for HFD
    # We can use the cumulative distance traveled
    path_distance = np.cumsum(np.linalg.norm(np.diff(xy_coords, axis=0), axis=1))
    
    hfd = calculate_hfd(path_distance)
...
# --- SAC Agent (with more readable `apply_suggestions`) ---
class ReplayBuffer:
...
class SACAgent:
    ...
    def select_action(self,s,eval=False):
        s=torch.tensor(s,device=device,dtype=torch.float32).unsqueeze(0);m,ls=self.actor(s);d=Normal(m,ls.exp());a=torch.tanh(m) if eval else torch.tanh(d.rsample());return(a.cpu().detach().numpy()[0]*self.action_scale.cpu().numpy())+self.action_bias.cpu().numpy()
    def update(self,B):
    ...
    def apply_suggestions(self,sg):
        if not sg: return
        print(f"  > Applying suggestions for agent...")
        if 'critic_lr' in sg:
            new_lr = sg['critic_lr']
...
class Trainer:
    def __init__(self, env_name, num_episodes, max_steps, eval_freq):
        ...
        self.agent = SACAgent(self.env) # Only one agent now
        self.analytics = ContinuousPirouetteAnalytics()
        self.shadow_detector = ContinuousShadowRegionDetector(self.analytics)

        # High-Score Memory: Stores (reward, fingerprint) tuples
        self.high_score_memory = deque(maxlen=20) 
        self.reward_history = []
    ...
    def train(self):
        for ep in range(self.num_episodes):
            ...
            for step in range(self.max_steps):
                ...
                s_, r, term, trunc, _ = self.env.step(a)
                done = term or trunc
                
                self.agent.buffer.push(s, a, r, s_, done)
                self.agent.update(batch_size=256) # Assuming a fixed batch size for simplicity
                
                s = s_
                ep_reward += r
                ep_state_history.append(s)
                if done: break
            
            # --- FRACTAL ANALYSIS & MEMORY ---
            # The "Senate" makes its judgment post-episode
            fingerprint = calculate_trajectory_fingerprint(ep_state_history)
            
            # Add to high-score memory
            self.high_score_memory.append((ep_reward, fingerprint))
            # Keep it sorted by reward, highest first
            self.high_score_memory = deque(sorted(self.high_score_memory, key=lambda x: x[0], reverse=True), maxlen=20)
            
            self.reward_history.append(ep_reward)
            print(f"Ep:{ep+1:03d} | Rw:{ep_reward:7.1f} | Fingerprint HFD: {fingerprint.get('hfd_xy', 0):.3f}")

            # Every N episodes, run the 'shadow' analysis and print high scores
            if (ep + 1) % EVAL_FREQUENCY == 0:
                print("\n--- Periodic Analysis & High Scores ---")
                ...
                print("  Top 5 Episodes in Memory:")
                for i, (score, fp) in enumerate(list(self.high_score_memory)[:5]):
                    print(f"    {i+1}. Score: {score:.2f}, HFD: {fp.get('hfd_xy', 0):.3f}")
                print("-" * 40 + "\n")
        ...
        for name, agent in self.agents.items():
            ...
            for i in range(EVAL_EPISODES):
                ...
                for _ in range(self.max_steps):
                    ...
                    a = agent.select_action(s, eval=True)
    ...
    def plot_rewards(self):
        ...
        for name, history in self.reward_history.items():
            ...
            if name == 'hydra_avg':
                ...
                x_vals = np.arange(len(self.reward_history['hydra_avg']))[list(self.agents.keys()).index(name)::len(self.agents)]
...
if __name__ == "__main__":
    trainer = Trainer(ENV_NAME, NUM_EPISODES, MAX_STEPS_PER_EPISODE, EVAL_FREQUENCY)
    trainer.train()