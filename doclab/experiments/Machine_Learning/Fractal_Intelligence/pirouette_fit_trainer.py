...
class PirouetteFITTrainer:
    def __init__(self, env_name, num_episodes, max_steps):
        ...
        self.inversion_mode, self.last_sigma = False, 0
    ...
    def train(self):
        ...
        for ep in range(1, self.num_episodes + 1):
            ...
            if self.inversion_mode: print(f"  -> Ep:{ep:03d} starts in ACTION INVERSION mode.")
            ...
            for step in range(1, self.max_steps + 1):
                ...
                total_reward = r_env + flux_reward + r_manifold + risk_bonus
            ...
            self.inversion_mode = True if ep_r_env_normalized < INVERSION_THRESHOLD and not self.inversion_mode else False
            ...
            if ep % PROPHET_QUIZ_INTERVAL == 0:
                ...
                print(f"Ep:{ep:04d} | Env Reward:{ep_r_env_normalized:8.2f} | Predictive Span (σ): {current_sigma:02d}/{PROPHET_PREDICTION_HORIZON}")
    ...
    def run_evaluation_and_manage_pool(self, ep):
        total_rewards, episode_lengths = [], []
        for i in range(EVAL_EPISODES):
            s_eval, _ = self.env.reset(seed=SEED * 100 + i); ep_r, steps_taken = 0, 0
            for _ in range(self.max_steps):
                a = self.agent.select_action(s_eval, eval=True)
                s_eval, r, term, trunc, _ = self.env.step(a)
                ep_r += r; steps_taken += 1
        ...
        else: self.consecutive_bad_cycles = 0
