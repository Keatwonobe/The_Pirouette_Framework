"""
Bilateral Hemisphere Agent - V3 (Memory Safe)
====================================================
- OBS_DIM: 105 (Corrected for Ant-v5)
- SAFE MEMORY: Caps workers at 8.
- POOL RECYCLING: Shuts down workers before Torch runs.
- BOMB-PROOF: Auto-saves and resumes.
"""
...
# Reduce max workers to prevent "WinError 1455" (Page File Full)
# 8 is usually safe. 16 crashes most Windows machines with Ant-v5.
MAX_WORKERS = 8 

PARAM_DIM = (H1 * OBS_DIM + H1) + (H2 * H1 + H2) + (ACT_DIM * H2 + ACT_DIM)
CHECKPOINT_FILE = "bilateral_checkpoint_v3.pkl"
...
class GracefulKiller:
    ...
    def exit_gracefully(self, signum, frame):
        print("\n\n[!] TERMINATION SIGNAL RECEIVED. FINISHING CYCLE THEN SAVING...")
        self.kill_now = True

# ============================================================================
# LEFT HEMISPHERE: PURE NUMPY
# ============================================================================
_worker_env = None
...
class LeftHemisphere:
    ...
    def ensure_pool(self):
        ...
        if self.pool is None:
            n_workers = min(MAX_WORKERS, max(1, cpu_count() - 2))
            print(f"[System] Spawning {n_workers} Numpy Workers...")
            self.pool = Pool(processes=n_workers, initializer=worker_init, maxtasksperchild=10)
            
    def release_pool(self):
        """Kill pool to free memory for PyTorch"""
        if self.pool is not None:
            print("[System] Releasing Workers to free RAM...")
            self.pool.terminate()
            self.pool.join()
            self.pool = None
            gc.collect()
    ...
    def step(self):
        self.gen += 1
        self.ensure_pool() # Make sure we have workers
        ...
        try:
            results = res_async.get(timeout=99999)
        except TimeoutError:
...
def load_checkpoint(left, right):
    ...
    # We can't set right state here if it's not initialized, 
    # but Right is lazy-loaded in main loop.
    # We will return the state data and load it after Right init.
    print(f"Resumed from Cycle {data['cycle']}")
    return data['cycle'] + 1, data.get('right_state')

def run_bilateral_training(cycles=100, left_gens=10, right_updates=3):
    killer = GracefulKiller()
    
    print("=" * 60)
    print(f"BILATERAL HEMISPHERE - MEMORY SAFE (Max Workers: {MAX_WORKERS})")
    print("=" * 60)
    ...
    # Checkpoint logic
    start_cycle = 1
    right_state_pending = None
    
    if os.path.exists(CHECKPOINT_FILE):
        # We need to pre-load to get start cycle
        with open(CHECKPOINT_FILE, 'rb') as f:
            data = pickle.load(f)
        
        print(f"Resume from Cycle {data['cycle']}? (y/n): ", end="")
        try: choice = input().lower().strip()
        except: choice = 'y'
        
        if choice == 'y':
            left.set_state(data['left_state'])
            start_cycle = data['cycle'] + 1
            right_state_pending = data['right_state']

    # Env for Right Hemisphere (Main Thread)
    env = gym.make("Ant-v5", terminate_when_unhealthy=True)
    
    # Right Hemisphere (Lazy Init)
    right = None
    ...
    try:
        for cycle in range(start_cycle, cycles + 1):
            ...
            # CRITICAL MEMORY FIX: Kill workers before loading Torch
            left.release_pool()
            
            if killer.kill_now: break

            # ==========================
            # RIGHT HEMISPHERE (Torch)
            # ==========================
            # Initialize Right if needed (Loads Torch)
            if right is None:
                print("[System] Initializing PyTorch Hemisphere...")
                right = RightHemisphere()
                if right_state_pending:
                    right.set_state(right_state_pending)
                    right_state_pending = None
        ...
        if killer.kill_now and right is not None:
            save_checkpoint(cycle, left, right)
