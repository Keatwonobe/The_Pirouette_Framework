...
class WendigoAgent:
    def __init__(self, env: gym.Env, seed: int = 42):
        ...
        self.gold_buffer = []  # keep last N golds in RAM
...
def main():
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # sort: 1) score desc, 2) dark asc
        top_eps.sort(key=lambda e: (-e["score"], e["dark"]))

        # keep only best 15
        top_eps = top_eps[:15]

        # keep your old dynamic threshold logic, but now from the structures
        avg_top = sum(e["score"] for e in top_eps) / len(top_eps)
        dyn_threshold = int(avg_top * 0.75)

        # 3.5 (your periodic save / housekeeping can stay here)
        if ep % 10 == 0:
        ...
        if is_gold:
            ...
            # --- 3.5 PERIODIC HOUSEKEEPING / CHECKPOINT (← your new code goes here) ---
            if ep % 10 == 0:
                # example: save model + print a compact state snapshot
                agent.sac.save(f"wendigo_sac_ep{ep}")
                # optional: also persist gold window
