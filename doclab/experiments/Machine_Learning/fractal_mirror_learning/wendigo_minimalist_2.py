...
def main():
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # ---------------------------------------------------
        # we turn the episode into sliding windows:
        # from obs[t] → [dr[t+1], ..., dr[t+H]]
        epi_len = len(ep_obs)
        if epi_len > 2:
            for t in range(epi_len - 1):
                x = ep_obs[t]
                future = []
                for k in range(1, SPAN_HORIZON + 1):
                    idx = t + k
                    if idx < len(ep_dr):
                        future.append(ep_dr[idx])
                    else:
                        # pad with last seen DR
                        future.append(ep_dr[-1])
                future = np.array(future, dtype=np.float32)
                prophet.train_step(x, future)

            # measure span on first point in episode
            x0 = ep_obs[0]
            y0 = []
            for k in range(1, SPAN_HORIZON + 1):
                if k < len(ep_dr):
            ...
            sigma = prophet.measure_predictive_span(x0, y0, rel_err_thresh=0.1)
        else:
            sigma = last_sigma  # no data, carry forward
