...
EPS_START = 0.50  # <<< FIX: Increased initial exploration to break out of the valley
EPS_END   = 0.05
...
# Training scheduling is now dynamic, see main loop
WEAVER_RATIO = 0.25
ANCHOR_RATIO = 0.25
...
class PirouetteAC:
    ...
    def update(self, batch, batch_weaver=None, batch_anchor=None):
        ...
        # Ensure batch is exactly BATCH_SIZE, handling cases where sources are small
        if not combined: return
        if len(combined) < BATCH_SIZE:
             combined += random.choices(combined, k=BATCH_SIZE - len(combined))
...
def main():
    ...
    for ep in range(1, TOTAL_EPISODES + 1):
        ...
        for t in range(1, MAX_STEPS + 1):
            ...
            ddr_n = dr_n - prev_dr_n  # <<< FIX: Correct ddr calculation ensures closure reward works
            ...
            prev_logits, prev_dr_n = cur_logits, dr_n # <<< FIX: Track previous normalized DR for next step's ddr calc
            obs = next_obs
            if done: break
        ...
        if len(replay) >= BATCH_SIZE:
            # <<< FIX: More updates for longer episodes, ensures sufficient learning
