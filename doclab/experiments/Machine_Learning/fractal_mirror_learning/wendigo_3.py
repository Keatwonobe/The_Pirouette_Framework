#!/usr/bin/env python3
"""
Wendigo (discrete-wrapped) v3
- adds predictive head
- adds Pirouette-style Lagrangian detector
- adds autopoietic cycle that reorders episodes and replays low-DR steps
- keeps tie-break on dark residue in leaderboard
"""
...
class WendigoAgent:
    ...
    def sharpen_with_whetstones(self, transitions: List[Tuple]):
        ...
        for (obs, action, next_obs, reward, dark, done) in transitions:
            # >>> annealed bonus: the cleaner, the more bonus, but scaled by temperature
    ...
    # >>> NEW: Pirouette-style episode Lagrangian
    def compute_episode_lagrangian(self, score, mean_dr, vigor_ratio):
        dark_term = 120.0 * mean_dr
        vigor_term = 5.0 * max(0.0, vigor_ratio - 0.55)
        L = score - dark_term - vigor_term
        return float(L)

    # >>> NEW: predictive head trainer
    def train_predictive_head(self, mean_dr, vigor_ratio, ep_len_norm,
                              dyn_threshold_norm, coh_pred_norm,
                              true_score, true_lagrangian):
        x = th.tensor([[mean_dr, vigor_ratio, ep_len_norm,
                        dyn_threshold_norm, coh_pred_norm]], dtype=th.float32, device=device)
        y = th.tensor([[true_score, true_lagrangian]], dtype=th.float32, device=device)
        out = self.predictive_head(x)
        loss = ((out - y) ** 2).mean()
        self.predictive_opt.zero_grad()
        loss.backward()
        self.predictive_opt.step()
        return float(loss.item()), float(out[0, 0].item()), float(out[0, 1].item())
    ...
    def run_autopoietic_cycle(self):
        ...
        ranked = sorted(
            self.auto_ring,
            key=lambda e: (-e["lag_pred"], e["mean_dr"])
        )
...
def main():
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # >>> normalize for predictive head
        dyn_threshold_norm = min(1.2, dyn_threshold / 500.0)
        coh_pred_norm = min(1.2, coh_pred / 500.0)

        # >>> true episode L
        true_L = agent.compute_episode_lagrangian(score, mean_dr, vigor_ratio)

        # >>> train predictive head (this is the *new* thing you wanted)
        ph_loss, score_hat, lag_hat = agent.train_predictive_head(
            mean_dr,
            vigor_ratio,
            ep_len_norm,
            dyn_threshold_norm,
        ...
        # GOLD WINDOW (same logic as before)
        ...
        # >>> NEW: register this episode into the autopoietic ring
        ...
        # >>> NEW: actually RUN the autopoietic cycle here
        # THIS is the spot you asked about.
        # It's after we've computed all episode-level stats,
        # after we've updated gold + leaderboard,
        # and before we print/log. That way it's still part of the same "cosm" pass.
        if ep % 4 == 0:
        ...
        print(
            ...
            f"| PredHead: scorê={score_hat:.1f} L̂={lag_hat:.1f} (loss={ph_loss:.3f})"
        )
        if gold.buffer:
