...
class KaleidoscopeMemory:
    """
    Multi-slot, but now with:
      - per-entry 'age'
      - global 'tick' to age everything every N episodes
      - forced insert for near-threshold episodes
    """
    ...
    def _sort_bin(self, arr):
        # newer entries get tiny bias so we don't freeze
        arr.sort(key=lambda c: (
            -1 if c["coherent"] else 0,
            c["residue"],
            -c["score"],
            -c.get("freshness", 0.0)  # newer first
        ))

    def _insert_candidate(self, key, candidate, force=False):
        arr = self.kaleidoscope.get(key, [])
        if force and len(arr) >= self.max_per_key:
            # drop the stalest / worst one
            arr = arr[:-1]  # after sort we'll reinsert
    ...
    def learn_from_history(self, episode_history, episode_score, coherent_flag,
                           dyn_threshold, force_radius=20):
        """
        force_radius: if episode_score >= dyn_threshold - force_radius,
        we allow forced overwrite to prevent permanent freeze.
        """
        updates = 0
        near_threshold = episode_score >= (dyn_threshold - force_radius)
        for obs_aug, action_arr, step_dark in episode_history:
            ...
            candidate = {
                ...
                "freshness": float(self.global_step)
            }

            before = self.kaleidoscope.get(key, [])
            self._insert_candidate(key, candidate, force=near_threshold)
            after = self.kaleidoscope.get(key, [])
            if len(after) > len(before) or after[0] is candidate:
                updates += 1
        return updates

    def age_all(self, factor=1.02):
        """
        Called every N episodes.
        Slightly worsen stored residues so new stuff can get in.
        """
        for key, arr in self.kaleidoscope.items():
            for entry in arr:
                entry["residue"] *= factor
        # re-sort
        for key, arr in self.kaleidoscope.items():
            self._sort_bin(arr)
...
def main():
    ...
    for ep in range(num_episodes):
        ...
        while not terminated and not truncated:
            ...
            def percentile_threshold(scores, p=0.75, floor=120):
                if not scores:
                    return floor
                arr = np.array(scores, dtype=np.float32)
                th = np.percentile(arr, p*100.0)
                return max(floor, int(th))

            # ...
            dyn_threshold = percentile_threshold(top_scores, p=0.70, floor=120)
        ...
        if (ep + 1) % 50 == 0:
            agent.kaleidoscope.age_all(factor=1.02)
