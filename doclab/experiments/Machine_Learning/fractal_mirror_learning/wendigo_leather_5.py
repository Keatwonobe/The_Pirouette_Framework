...
class SnailProbeTool:
    def __init__(
        ...
        self.best_seen_score = -1e9  # <-- for monotonic snail
...
def train_task(task):
    ...
    for ep in range(1, task["max_episodes"] + 1):
        ...
        # snail baseline = actual episode env return (not 200 steps)
        ...
        top_scores.append(ep_steps)
        top_scores.sort(reverse=True)
        top_scores = top_scores[:15]
        avg_top = sum(top_scores) / len(top_scores)

        print(
            f"Ep {ep:04d} | Steps={ep_steps:4d} | EnvR={ep_env_return:8.1f} | "
            f"Span={span:2d}/{task['horizon']} | Top15={avg_top:7.1f}"
        )

        if task["name"] == "Pendulum" and ep_env_return >= task["pendulum_mastery"]:
