...
class ValleyCrossing:
    ...
    def reconstruction_quality(self) -> float:
        ...
        return 0.0
    @property
    def valley_score(self) -> float:
        # depth × reconstruction; chatter will be added from signature
        return max(0.0, self.coherence_drop) * max(0.0, self.reconstruction_quality)
...
def main():
    ...
    for idx, v in enumerate(valleys):
        sig = ValleySignatureExtractor.extract_signature(v, df)
        valley_rows.append({
            'valley_id': idx,
            'entry_idx': v.entry_idx,
            'nadir_idx': v.nadir_idx,
            'exit_idx': v.exit_idx,
            'duration': v.valley_duration,
            'DR_increase': v.DR_increase,
            'coherence_drop': v.coherence_drop,
            'net_coherence_gain': v.net_coherence_gain,
            'reconstruction_quality': v.reconstruction_quality,
            'valley_chatter': sig['valley_chatter'],
            'coherence_overshoot': sig['coherence_overshoot'],
            'is_successful': v.is_successful,
            'is_hemispheric': v.is_hemispheric_split,
        })

    valley_summary = pd.DataFrame(valley_rows)
    valley_summary.to_csv(args.output_dir / 'valley_summary.csv', index=False)

    # Rank by “most valley”
    valley_summary = valley_summary.sort_values(
        by=['coherence_drop', 'reconstruction_quality'],
        ascending=[False, False]
    )
    print("\nTop 5 'most valley' crossings:")
    print(valley_summary.head(5))
