...
# --------------------------------------------------------------
    def run_episode(self):
        ...
        The brain state is now sampled ONCE per episode and held constant.
        """
        ...
        # <--- CHANGE 1: BRAIN IS SAMPLED ONCE, OUTSIDE THE LOOP ---
        ...
        # Log the persistent brain state
        for k in brain_log:
            if k in sample:
                brain_log[k].append(float(sample[k]))
        
        # (This is now a single-value-list, but we keep the logic 
        # for the mean() call at the end to work)
        
        DR_t = float(sample["DR"]) # Get the single DR for this episode
        ...
        for t in range(self.cfg.max_steps):
            ...
            # --- 1) Brain step ---
            # <--- CHANGE 2: WE NO LONGER SAMPLE THE BRAIN PER-STEP ---
            # We use the persistent brain_feat and brain_t from above
            observations.append(obs.copy())
            brain_features.append(brain_feat.copy()) # Store the same features
            
            # --- 2) Policy forward ---
            obs_t = torch.as_tensor(obs, dtype=torch.float32, device=self.device).unsqueeze(0)
            
            with torch.no_grad():
                # Use the *same* brain_t tensor for every step
            ...
            if self.mode in ("brain", "fusion"):
                # <--- CHANGE 3: DELTA-DR LOGIC MODIFIED ---
                # We now compare the *current* step's obs/state 
                # to the *initial* brain-state DR.
                # A more common way is to reward "achieving" that state.
                # For simplicity, we'll use the original logic,
                # which now just checks if the initial DR has changed.
                if previous_DR is not None:
                    # This logic will only run once now, which is fine.
                    # A better model might be to check `sample["DR"]` 
                    # against a *new* sample's DR, but that's complex.
                    # Let's stick to the original logic's *intent*.
                    dDR = DR_t - previous_DR 
                    r_brain = max(0.0, -dDR) # reward reducing DR
                previous_DR = DR_t 
                # Note: This 'brain' reward will now be very sparse,
                # which is probably good. It lets the env reward dominate.
