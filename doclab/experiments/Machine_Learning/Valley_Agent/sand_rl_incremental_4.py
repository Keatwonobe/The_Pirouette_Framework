...
class Config:
    ...
    # Trajectory scaling (Keaton's coherence island hopping)
    plateau_threshold: float = 900.0  # Ant: 900, Humanoid: 1500
    plateau_patience: int = 100  # Episodes to wait before void crossing
...
class ActorCriticTrainer:
    ...
    NEW: Trajectory scaling for coherence island hopping
    - Detects plateaus (stuck on local maximum)
    - Switches to exploration mode (cross void to next island)
    - Scales trajectory length (120-step valleys vs 40-step)
    """
    ...
    def __init__(self, policy: GaussianPolicy, config: Config, device: str = 'cpu'):
        ...
        # Save base learning rates for trajectory scaling
        self.base_policy_lr = config.lr
        self.base_value_lr = config.lr * 3
        ...
        # Trajectory scaling (NEW - Keaton's coherence island hopping)
        self.plateau_threshold = 900.0  # Ant: 900, Humanoid: 1500
        self.patience_episodes = 100
        self.improvement_threshold = 50.0
        self.plateau_start_episode = None
        self.episodes_since_improvement = 0
        self.trajectory_mode = "EXPLOITATION"  # or "EXPLORATION"
        ...
        self.base_entropy = 0.01  # For scaling back up in exploration mode
    ...
    def train_episode(self, env, metrics_tracker: Optional[SandMetrics] = None):
        ...
        # Trajectory scaling check (NEW - Keaton's coherence island hopping)
        recent_avg = np.mean(self.recent_rewards_for_buffer[-20:]) if len(self.recent_rewards_for_buffer) >= 20 else total_reward
        
        # Check for improvement
        if recent_avg > self.best_avg_reward + self.improvement_threshold:
        ...
        # Detect plateau and switch modes
        if recent_avg >= self.plateau_threshold and self.plateau_start_episode is None:
            self.plateau_start_episode = 0  # Will be set by episode counter externally
        
        if self.plateau_start_episode is not None and self.episodes_since_improvement >= self.patience_episodes:
            if self.trajectory_mode == "EXPLOITATION":
                # Switch to EXPLORATION mode
                self.trajectory_mode = "EXPLORATION"
                self.mode_switches += 1
                
                # SCALE TRAJECTORY - Keaton's key insight
                print(f"\n🌊 VOID CROSSING MODE ACTIVATED (attempt #{self.mode_switches})")
                print(f"   Stuck at {recent_avg:.0f} for {self.episodes_since_improvement} episodes")
                print(f"   Scaling trajectory to bridge coherence gap:")
                
                # Triple entropy (3x exploration)
                self.entropy_coef = min(0.03, self.base_entropy * 3.0)
                print(f"   - Entropy: {self.entropy_coef:.4f} (3x baseline)")
                
                # Halve learning rates (more careful)
                for g in self.policy_optimizer.param_groups:
                    g['lr'] = self.base_policy_lr * 0.5
                for g in self.value_optimizer.param_groups:
                    g['lr'] = self.base_value_lr * 0.3
                print(f"   - Policy LR: {self.policy_optimizer.param_groups[0]['lr']:.6f} (0.5x)")
                print(f"   - Value LR: {self.value_optimizer.param_groups[0]['lr']:.6f} (0.3x)")
                
                # Clear buffer to forget plateau strategies
                self.episode_buffer.clear()
                print(f"   - Buffer cleared (forget plateau strategies)")
                
        elif self.trajectory_mode == "EXPLORATION" and recent_avg > self.best_avg_reward + self.improvement_threshold:
            # Found new island! Switch back to EXPLOITATION
            self.trajectory_mode = "EXPLOITATION"
            
            print(f"\n🏝️  NEW COHERENCE ISLAND DISCOVERED!")
            print(f"   New best: {recent_avg:.0f} (was: {self.best_avg_reward:.0f})")
            print(f"   Void successfully crossed! Returning to normal trajectory:")
            
            # Reset to normal parameters
            self.entropy_coef = self.base_entropy
            print(f"   - Entropy: {self.entropy_coef:.4f} (baseline)")
            
            for g in self.policy_optimizer.param_groups:
                g['lr'] = self.base_policy_lr
            for g in self.value_optimizer.param_groups:
                g['lr'] = self.base_value_lr
            print(f"   - Policy LR: {self.policy_optimizer.param_groups[0]['lr']:.6f} (normal)")
            print(f"   - Value LR: {self.value_optimizer.param_groups[0]['lr']:.6f} (normal)")
...
class IncrementalRLTrainer:
    ...
    def __init__(self, config: Config, output_dir: Path):
        ...
        # Configure trajectory scaling parameters
        if hasattr(config, 'plateau_threshold'):
            self.trainer.plateau_threshold = config.plateau_threshold
        if hasattr(config, 'plateau_patience'):
            self.trainer.patience_episodes = config.plateau_patience
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=900.0,
        help="Reward level indicating coherence island reached (default: 900 for Ant, try 1500 for Humanoid)"
    )
    parser.add_argument(
        '--plateau-patience',
        type=int,
        default=100,
        help="Episodes to wait for improvement before void crossing mode (default: 100)"
    )
