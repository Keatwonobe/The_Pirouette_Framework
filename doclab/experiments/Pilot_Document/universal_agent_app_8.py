...
def render_transcript_watcher():
    ...
    try:
        if os.path.exists(TRANSCRIPT_FILE):
            ...
            st.warning("⏳ Waiting for live transcription... (Start active_listener.py)")
            return
    except Exception as e:
        st.error(f"❌ Error reading transcript: {e}")
        return
    
    # Process new lines for KB search
    if len(lines) > st.session_state.last_processed_line:
        ...
        # Update accumulated links (prevent duplicates, move to top)
    ...
    if lines:
        ...
        st.text_area("Live Transcript", value=log_text, height=200, disabled=True, 
                    label_visibility="collapsed", key="transcript_display")
    else:
