...
# ==========================================
# FRAGMENTS (The Magic Logic)
# ==========================================

@st.fragment(run_every=1)
def render_knowledge_stream():
    """
    This fragment runs independently every 1 second.
    It acts as the 'Consumer' for the microphone queue.
    """
    # 1. PROCESS QUEUE (The Logic)
    if st.session_state.listening_active and 'listener_queue' in st.session_state:
        try:
            # Loop to drain the queue of all new phrases
            while not st.session_state.listener_queue.empty():
                ...
                # Update Global State (Last Transcript)
                st.session_state.last_transcript = transcript
                
        except queue.Empty:
            pass

    # 2. RENDER UI (The Visuals)
...
def render_last_heard_box():
    """
    This fragment also runs every 1 second to update the blue box
    in the middle column without reloading the chat.
    """
...
with middle:
    ...
    # Chat interface (Standard Streamlit - DOES NOT REFRESH AUTOMATICALLY)
