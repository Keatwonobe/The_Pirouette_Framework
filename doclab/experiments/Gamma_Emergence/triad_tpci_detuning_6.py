# File: triad_tpci_detuning_6_split.py
# This version uses preload=False and saves each combination (triad+load)
# to a separate JSON file to drastically reduce memory usage.
...
# <-- MODIFIED: Function signature changed to accept an output *directory*
def process_file(eeg_path, ev_path, out_dir, args):
    ...
    # <-- MODIFIED: CRITICAL MEMORY FIX
    # Change preload=True to preload=False. This stops MNE from loading
    # all epochs into RAM. Data will be read from disk as needed.
    # ...
    ...
    # <-- MODIFIED LINE:
    # Check the length of the 'events' array, not the 'epochs' object.
    # This also correctly uses the 'min_epochs' argument.
    ...
    if loads is None:
        # <-- MODIFIED: CRITICAL MEMORY FIX
        # We must iterate over epochs one by one to calculate power
        # instead of calling .get_data() on all of them at once.
        print("  -> Calculating proxy load (beta band power) epoch-by-epoch...")
        bp = []
        # We can just iterate over the epochs object.
        # With preload=False, MNE loads each one from disk.
        for ep_data in epochs: # <-- THIS IS THE FIX
            # band_power expects (n_epochs, n_chans, n_samples)
            # So we add a new axis to make it (1, n_chans, n_samples)
            ep_data_batch = ep_data[np.newaxis, :, :] 
            power_val = band_power(ep_data_batch, sf, 15.0, 30.0) # Returns np.array([value])
    ...
    processed_something = False # <-- MODIFIED: Flag to track if we do any work
    ...
    for (f1, f2, f3_nom) in triads_in:
        for L in sorted(np.unique(loads)):
            ...
            # Define a unique path for this specific combination's result
            triad_str = f"{f1}-{f2}-{f3_nom}".replace('.', 'p') # Make filename safe
            ...
            # <-- MODIFIED: This is now memory-efficient!
            # Because epochs.preload=False, .get_data() only reads
            # the epochs specified by [idx] from the disk.
            ...
            # --- MODIFIED: Pre-calculate all phi3 variants ---
            ...
            # --- MODIFIED: Calculate real curve using pre-calced list ---
            curve = []
            # Use enumerate(phi3_list) to get the phi3 data
            ...
            if np.ptp(curve) > args.precheck_threshold:
                ...
                print(f"     ...starting {args.perm} permutations...")
                
                for i in range(args.perm): 
                    if (i+1) % 50 == 0: print(f"     ...perm {i+1}/{args.perm}")
                    phi2_null = phase_shuffle(phi2, rng)
                    
                    # --- MODIFIED: Use pre-calced list, don't re-calculate ---
                    cnull = []
                    # Use enumerate(phi3_list) to get the pre-calculated phi3
                    for j, phi3 in enumerate(phi3_list): 
            ...
            # <-- MODIFIED: Create a dictionary for this *single result*
            result_dict = dict(
            ...
            # --- MODIFIED: CHECKPOINT SAVE ---
            # Save just this one result to its specific file
            with open(out_path_combo, "w") as f:
                json.dump(result_dict, f, indent=2)
            # --- END MODIFIED CHECKPOINT SAVE ---
            
            processed_something = True # Mark that we saved a file

    return processed_something # <-- MODIFIED: Return True if we saved anything
...
def main():
    ...
    # <-- MODIFIED: Create a dedicated *directory* for this file's results
    ...
    # <-- MODIFIED: No need to load existing results; the function
    # will check for individual files.
    
    if args.overwrite:
        print(f"i --overwrite enabled. Existing individual result files in {out_dir_for_file.name} will be overwritten.")
        # Note: This only overwrites files it *computes*. It won't clear
        # the directory of old results from different parameters.

    print(f"▶ Processing: {eeg_path.name}...")
    print(f"  Results will be saved in: {out_dir_for_file}")
    try:
        # <-- MODIFIED: Pass the new output directory to the function
        ...
        # raise e # You may want to comment this out during a big batch
