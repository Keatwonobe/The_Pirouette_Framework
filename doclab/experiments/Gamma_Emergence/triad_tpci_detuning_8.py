# File: triad_tpci_detuning_8_parallel_init.py
# This version uses a Pool initializer to load large data into
# worker processes ONCE, solving the [WinError 1450].
...
# (Helper functions like read_raw_any, tpci, etc. are unchanged)
...
# This function is called ONCE by each worker process
def init_worker(phi1_init, phi2_init, phi3_list_init):
    """
    Initializes the worker process by setting the large
    data arrays as global variables within that worker.
    """
    # These 'global' variables are *only* global to this
    # specific worker, not the main process.
    global worker_phi1, worker_phi2, worker_phi3_list
    worker_phi1 = phi1_init
    worker_phi2 = phi2_init
    worker_phi3_list = phi3_list_init


# <--- MODIFIED: Worker function now uses globals ---
# It no longer accepts the large phi arrays as arguments
def run_one_permutation(perm_index, detune, base_seed):
    """
    Runs a single permutation using the globally-stored phi arrays.
    """
    # 1. Access the large arrays from the worker's global scope
    # (These were set by init_worker)
    global worker_phi1, worker_phi2, worker_phi3_list
    ...
    cnull = []
    # We now access worker_phi1 and worker_phi3_list
    for phi3 in worker_phi3_list: 
        v = tpci(worker_phi1, phi2_null, phi3)
...
def process_file(eeg_path, ev_path, out_dir, args):
    ...
    if args.load_col and args.load_col in ev.columns:
        ...
        uniq = pd.unique(loads[~pd.na(loads)])
        if len(uniq) >= 2:
    ...
    for (f1, f2, f3_nom) in triads_in:
        for L in sorted(np.unique(loads)):
            ...
            if np.ptp(curve) > args.precheck_threshold:
                ...
                # <--- MODIFIED: 'partial' function no longer sends phi arrays ---
                # It's now a very small object.
                ...
                # <--- MODIFIED: Pool now uses 'initializer' and 'initargs' ---
                # This sends the large arrays to each worker *once*.
                with multiprocessing.Pool(processes=n_cpus,
                                          initializer=init_worker,
                                          initargs=(phi1, phi2, phi3_list)) as pool:
                    
                    perm_indices = range(args.perm)
                    
                    # The rest of this loop is the same, but now the 'put(task)'
                    # call that was failing is only sending tiny integers.
            ...
            # (Rest of the file-saving logic is unchanged)
