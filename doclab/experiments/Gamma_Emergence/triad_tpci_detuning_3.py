...
def process_file(eeg_path, ev_path, args):
    ...
    if args.roi:
        # ## MODIFICATION: Corrected roi -> args.roi
    ...
    for (f1, f2, f3_nom) in triads_in:
        for L in sorted(np.unique(loads)):
            ...
            ## -------------------------------------------------------------------
            ## MODIFICATION: Pre-computation check to save time
            ## -------------------------------------------------------------------
            status = "computed"
            bw_half, fitpars, pval = np.nan, None, None
            # Check if the curve's peak-to-peak value is above a threshold
            ...
            out_rows.append(dict(
                ...
                task=task,
                triad=(f1, f2, f3_nom),
                status=status, # ## MODIFICATION: Added status field
...
# --------------------------
# Directory walker (No changes here)
# --------------------------
...
def main():
    ...
    # ## MODIFICATION: New arguments for checkpointing and optimization
    ...
    # ## MODIFICATION: Main loop with checkpointing
    for eeg_path, ev_path in entries:
        # Define a unique output name for this specific task file
        ...
        try:
            ...
            if rows:
                ...
                print(f"  ✓ Saved {len(rows)} results to {out_path.name}")
            else:
                print(f"  ! No results produced for {eeg_path.name}")

        except Exception as e:
            warnings.warn(f"  ✗ FAILED {eeg_path.name} -> {e}")

    # ## MODIFICATION: Post-processing step to create summary file
    print("\n--- Analysis complete. Consolidating results... ---")
    all_rows = []
    for json_file in outdir.glob("sub-*_ses-*_task-*_tpci.json"):
    ...
    summary_path = outdir/"_summary_tpci_detuning_all.json"
    with open(summary_path, "w") as f:
        json.dump(all_rows, f, indent=2)
    print(f"Wrote {len(all_rows)} total results to {summary_path}")

    # ## MODIFICATION: Final analysis moved here, unchanged logic
    from collections import defaultdict
    agg = defaultdict(list)
    for r in all_rows:
        bw = r.get("bandwidth_half") # Use .get() for safety
        if bw is not None:
    ...
    for triad, pairs in agg.items():
        ...
        if len(np.unique(Ls)) < 2:
            print(f"Triad {triad}: insufficient load variance for fit.")
            continue
        try:
            x = Ls**(-0.5)
            ...
            print(f"Could not fit law for triad {triad}: {e}")
