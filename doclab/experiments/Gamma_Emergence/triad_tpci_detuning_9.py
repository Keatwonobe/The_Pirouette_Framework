# File: triad_tpci_detuning_10_pool_pause.py
# This version adds a pause *before* the Pool is created
# to fix the hang after rapid 'skip' loops.
...
# (All helper functions, init_worker, and run_one_permutation are identical)
# ...
...
# <--- NEW: Worker Initializer Function ---
def init_worker(epL_init, sf_init, f1_init, f2_init, 
                f3_nom_init, detune_init, band_init):
    """
    Initializes the worker process by setting the raw epoch data
    and parameters as global variables.
    """
    global worker_epL, worker_sf, worker_f1, worker_f2
    global worker_f3_nom, worker_detune, worker_band
    
    # The ONLY large array
    worker_epL = epL_init
    
    # Parameters needed to calculate all phi
    worker_sf = sf_init
    worker_f1 = f1_init
    worker_f2 = f2_init
    worker_f3_nom = f3_nom_init
    worker_detune = detune_init
    worker_band = band_init

# <--- MODIFIED: Worker function now uses globals AND calculates phi3 ---
# <--- MODIFIED: Worker calculates phi1, phi2, AND phi3 ---
def run_one_permutation(perm_index, base_seed):
    """
    Runs a single permutation using the globally-stored epL data.
    It now calculates phi1, phi2, and phi3 on the fly.
    """
    # 1. Access the data from the worker's global scope
    global worker_epL, worker_sf, worker_f1, worker_f2
    global worker_f3_nom, worker_detune, worker_band
    
    # 2. Create a NEW RNG for this specific worker
    rng = np.random.default_rng(base_seed + perm_index)

    # 3. Calculate phi1 and phi2 (and phi2_null) INSIDE the worker
    # This avoids transferring them
    phi1 = bandpass_phase(worker_epL, worker_sf, 
                          worker_f1 - worker_band, worker_f1 + worker_band)
    phi2 = bandpass_phase(worker_epL, worker_sf, 
                          worker_f2 - worker_band, worker_f2 + worker_band)
    
    phi2_null = phase_shuffle(phi2, rng)
    
    # 4. Calculate the null curve
    cnull = []
    for df in worker_detune: 
        f3 = worker_f3_nom + df
        
        # Calculate phi3 on the fly
        phi3 = bandpass_phase(worker_epL, worker_sf, 
                              f3 - worker_band, f3 + worker_band)
        
        v = tpci(phi1, phi2_null, phi3)
        cnull.append(np.mean(v))
    
    # 5. Fit the curve and return the bandwidth
    bw_n, _ = halfwidth_gauss(worker_detune, cnull)
    ...
    for (f1, f2, f3_nom) in triads_in:
        for L in sorted(np.unique(loads)):
            ...
            if np.ptp(curve) > args.precheck_threshold:
                ...
                # <--- MODIFIED: 'partial' function no longer sends detune ---
                ...
                # <--- MODIFIED: Pool initargs now send raw materials, NOT phi3_list ---
                # This avoids the massive data transfer on Windows.
                initargs_tuple = (epL, sf, f1, f2, f3_nom, detune, args.band)
                
                with multiprocessing.Pool(processes=n_cpus,
                                          initializer=init_worker,
                                          initargs=initargs_tuple) as pool:
            ...
            print("     ...flushing memory & pausing 2s for OS cleanup...")
            try:
                del epL, phi1, phi2, phi3_list, result_dict, worker_func
                gc.collect()
            except Exception:
                pass
            time.sleep(2)
...
def main():
    ...
    # <--- This is the typo I fixed from _9.py ---
    ...
    task_match = re.search(r"task-([A-Za-z0-N_]+)", str(eeg_path.name)) #<-- Also fixed a typo here (0-9)
