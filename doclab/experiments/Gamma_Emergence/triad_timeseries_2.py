# File: triad_timeseries_batch.py
#
...
#
# This version has been modified to crawl a directory and batch
# process all found EEG/events.tsv pairs.

import os, re, json, pathlib, warnings, argparse
import numpy as np

# --- Helper Functions (Re-used from previous scripts) ---
# (These are all unchanged from your original script)
...
#
def main():
    ap = argparse.ArgumentParser(description="Triadic Time-Series 'Triangulator' Batch Agent")
    
    # --- Main Args ---
    # MODIFIED: Replaced eeg-path and events-path with search-dir
    ap.add_argument("--search-dir", required=True, help="Base directory to recursively search for EEG files (e.g., '..._eeg.bdf')")
    ap.add_argument("--outdir", required=True, help="Base output directory for all results")
    ...
    # --- NEW: Setup Search ---
    search_path = pathlib.Path(args.search_dir)
    outdir_base = pathlib.Path(args.outdir)
    
    # Find all potential EEG files recursively
    eeg_extensions = ['.bdf', '.set', '.edf']
    all_eeg_files = []
    for ext in eeg_extensions:
        # Assuming BIDS-like naming convention (e.g., ..._eeg.bdf)
        all_eeg_files.extend(search_path.rglob(f'*_eeg{ext}')) 

    if not all_eeg_files:
        print(f"No EEG files (*_eeg.bdf/set/edf) found in {search_path}")
        return

    print(f"▶ Found {len(all_eeg_files)} potential EEG files. Starting batch processing...")
    print(f"▶ Target Triad: {args.triad}")
    print(f"▶ Base Output Dir: {outdir_base}")
    
    success_count = 0
    fail_count = 0

    # --- NEW: Batch Processing Loop ---
    for eeg_path in all_eeg_files:
        print(f"\n--- Processing: {eeg_path.name} ---")
        
        # --- NEW: Find matching events.tsv file ---
        # Assumes BIDS: sub-01_task-nb_eeg.bdf -> sub-01_task-nb_events.tsv
        try:
            base_name = eeg_path.name.split('_eeg.')[0]
            ev_name = f"{base_name}_events.tsv"
            ev_path = eeg_path.with_name(ev_name)
            
            if not ev_path.exists():
                print(f"  ! Skipping: Could not find matching events file: {ev_name}")
                fail_count += 1
                continue
        except Exception as e:
            print(f"  ! Skipping: Error trying to find events file for {eeg_path.name}: {e}")
            fail_count += 1
            continue
            
        # --- Setup Output Directory (moved from old main) ---
        ...
        try:
            # Pass the found paths and the shared args to the processor
