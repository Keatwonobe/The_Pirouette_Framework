...
def main():
    ...
    # Arguments below are kept but not used in the chi2 function for now
    ...
    def line(label, a, b, units=""):
        print(f"{label:<16} {a:.6e} {units:<5} | target {b:.6e} {units}")

    print("\n=== Pirouette × NuFIT v6.0 (release .txt) ===")
    
    # --- NO Results ---
    print(f"\n[NO] mu={res['NO']['mu']:.6e} eV, p={res['NO']['p']:.4f}, q={res['NO']['q']:.4f},  χ²={res['NO']['chi2']:.3f}")
    line("Δm^2_21", res["NO"]["dm21_pred"], res["NO"]["dm21_target"], "eV^2")
    line("Δm^2_3ℓ", res["NO"]["dm3l_pred"], res["NO"]["dm3l_target"], "eV^2")
    print(f"m (eV): {', '.join(f'{x:.4e}' for x in res['NO']['m_eV'])}")
    print(f"Σm={res['NO']['Sigma_m']:.4e} eV; m_beta={res['NO']['m_beta']:.4e} eV; |m_bb|_max={res['NO']['m_bb_max']:.4e} eV")

    # --- IO (p,q frozen) Results ---
    print(f"\n[IO frozen] mu={res['IO_frozen']['mu']:.6e} eV (p,q from NO),  χ²={res['IO_frozen']['chi2']:.3f}")
    line("Δm^2_21", res["IO_frozen"]["dm21_pred"], res["IO_frozen"]["dm21_target"], "eV^2")
    line("Δm^2_3ℓ", res["IO_frozen"]["dm3l_pred"], res["IO_frozen"]["dm3l_target"], "eV^2")
    print(f"m (eV): {', '.join(f'{x:.4e}' for x in res['IO_frozen']['m_eV'])}")
    print(f"Σm={res['IO_frozen']['Sigma_m']:.4e} eV; m_beta={res['IO_frozen']['m_beta']:.4e} eV; |m_bb|_max={res['IO_frozen']['m_bb_max']:.4e} eV")

    # <-- NEW: IO (mu,p,q free) Results ---
    print(f"\n[IO free] mu={res['IO_free']['mu']:.6e} eV, p={res['IO_free']['p']:.4f}, q={res['IO_free']['q']:.4f},  χ²={res['IO_free']['chi2']:.3f}")
    line("Δm^2_21", res["IO_free"]["dm21_pred"], res["IO_free"]["dm21_target"], "eV^2")
    line("Δm^2_3ℓ", res["IO_free"]["dm3l_pred"], res["IO_free"]["dm3l_target"], "eV^2")
    print(f"m (eV): {', '.join(f'{x:.4e}' for x in res['IO_free']['m_eV'])}")
    print(f"Σm={res['IO_free']['Sigma_m']:.4e} eV; m_beta={res['IO_free']['m_beta']:.4e} eV; |m_bb|_max={res['IO_free']['m_bb_max']:.4e} eV")
