"""
Auto-generated from dictionary entry: DIRECT_INTERVENTION
...
class DirectIntervention(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"DIRECT_INTERVENTION:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="DIRECT_INTERVENTION:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"DIRECT_INTERVENTION:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.direct_intervention import DirectIntervention\n>>> # TODO: fill example"
