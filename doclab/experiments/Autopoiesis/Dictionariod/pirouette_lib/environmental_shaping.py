"""
Auto-generated from dictionary entry: ENVIRONMENTAL_SHAPING
...
class EnvironmentalShaping(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="ENVIRONMENTAL_SHAPING:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"ENVIRONMENTAL_SHAPING:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="ENVIRONMENTAL_SHAPING:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"ENVIRONMENTAL_SHAPING:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.environmental_shaping import EnvironmentalShaping\n>>> # TODO: fill example"
