"""
Auto-generated from dictionary entry: TOPOLOGICAL_PERSISTENCE
...
class TopologicalPersistence(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="TOPOLOGICAL_PERSISTENCE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"TOPOLOGICAL_PERSISTENCE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="TOPOLOGICAL_PERSISTENCE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"TOPOLOGICAL_PERSISTENCE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.topological_persistence import TopologicalPersistence\n>>> # TODO: fill example"
