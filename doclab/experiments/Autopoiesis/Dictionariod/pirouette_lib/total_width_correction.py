"""
Auto-generated from dictionary entry: TOTAL_WIDTH_CORRECTION
...
class TotalWidthCorrection(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="TOTAL_WIDTH_CORRECTION:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"TOTAL_WIDTH_CORRECTION:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="TOTAL_WIDTH_CORRECTION:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"TOTAL_WIDTH_CORRECTION:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.total_width_correction import TotalWidthCorrection\n>>> # TODO: fill example"
