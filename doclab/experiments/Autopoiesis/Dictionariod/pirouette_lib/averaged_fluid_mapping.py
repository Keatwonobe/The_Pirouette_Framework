"""
Auto-generated from dictionary entry: AVERAGED_FLUID_MAPPING
...
class AveragedFluidMapping(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="AVERAGED_FLUID_MAPPING:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"AVERAGED_FLUID_MAPPING:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="AVERAGED_FLUID_MAPPING:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"AVERAGED_FLUID_MAPPING:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.averaged_fluid_mapping import AveragedFluidMapping\n>>> # TODO: fill example"
