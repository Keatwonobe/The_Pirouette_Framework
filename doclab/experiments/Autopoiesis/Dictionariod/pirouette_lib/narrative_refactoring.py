"""
Auto-generated from dictionary entry: NARRATIVE_REFACTORING
...
class NarrativeRefactoring(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="NARRATIVE_REFACTORING:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"NARRATIVE_REFACTORING:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="NARRATIVE_REFACTORING:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"NARRATIVE_REFACTORING:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.narrative_refactoring import NarrativeRefactoring\n>>> # TODO: fill example"
