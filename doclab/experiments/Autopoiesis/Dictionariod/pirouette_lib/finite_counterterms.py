"""
Auto-generated from dictionary entry: FINITE_COUNTERTERMS
Term: FINITE_COUNTERTERMS
Symbol: 
Aliases: 
...
class FiniteCounterterms(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"FINITE_COUNTERTERMS:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="FINITE_COUNTERTERMS:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"FINITE_COUNTERTERMS:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.finite_counterterms import FiniteCounterterms\n>>> # TODO: fill example"
