"""
Auto-generated from dictionary entry: REVERSE_PARETO_ANALYSIS
...
class ReverseParetoAnalysis(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="REVERSE_PARETO_ANALYSIS:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"REVERSE_PARETO_ANALYSIS:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="REVERSE_PARETO_ANALYSIS:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"REVERSE_PARETO_ANALYSIS:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.reverse_pareto_analysis import ReverseParetoAnalysis\n>>> # TODO: fill example"
