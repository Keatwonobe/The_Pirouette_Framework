"""
Auto-generated from dictionary entry: INTERNAL_TEMPORAL_PRESSURE
...
class InternalTemporalPressure(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="INTERNAL_TEMPORAL_PRESSURE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"INTERNAL_TEMPORAL_PRESSURE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="INTERNAL_TEMPORAL_PRESSURE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"INTERNAL_TEMPORAL_PRESSURE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.internal_temporal_pressure import InternalTemporalPressure\n>>> # TODO: fill example"
