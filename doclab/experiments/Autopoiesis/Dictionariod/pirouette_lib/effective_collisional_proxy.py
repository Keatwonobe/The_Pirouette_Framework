"""
Auto-generated from dictionary entry: EFFECTIVE_COLLISIONAL_PROXY
...
class EffectiveCollisionalProxy(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="EFFECTIVE_COLLISIONAL_PROXY:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"EFFECTIVE_COLLISIONAL_PROXY:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="EFFECTIVE_COLLISIONAL_PROXY:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"EFFECTIVE_COLLISIONAL_PROXY:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.effective_collisional_proxy import EffectiveCollisionalProxy\n>>> # TODO: fill example"
