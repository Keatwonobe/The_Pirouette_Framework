"""
Auto-generated from dictionary entry: PROCEDURAL_CRUCIBLE
...
class ProceduralCrucible(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"PROCEDURAL_CRUCIBLE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="PROCEDURAL_CRUCIBLE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"PROCEDURAL_CRUCIBLE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.procedural_crucible import ProceduralCrucible\n>>> # TODO: fill example"
