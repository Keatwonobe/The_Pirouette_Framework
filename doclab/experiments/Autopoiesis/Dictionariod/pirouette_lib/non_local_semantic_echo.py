"""
Auto-generated from dictionary entry: NON_LOCAL_SEMANTIC_ECHO
...
class NonLocalSemanticEcho(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="NON_LOCAL_SEMANTIC_ECHO:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"NON_LOCAL_SEMANTIC_ECHO:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="NON_LOCAL_SEMANTIC_ECHO:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"NON_LOCAL_SEMANTIC_ECHO:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.non_local_semantic_echo import NonLocalSemanticEcho\n>>> # TODO: fill example"
