"""
Auto-generated from dictionary entry: HARMONIC_COMPATIBILITY
...
class HarmonicCompatibility(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="HARMONIC_COMPATIBILITY:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"HARMONIC_COMPATIBILITY:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="HARMONIC_COMPATIBILITY:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"HARMONIC_COMPATIBILITY:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.harmonic_compatibility import HarmonicCompatibility\n>>> # TODO: fill example"
