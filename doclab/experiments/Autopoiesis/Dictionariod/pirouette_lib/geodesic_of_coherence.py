"""
Auto-generated from dictionary entry: GEODESIC_OF_COHERENCE
...
class GeodesicOfCoherence(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="GEODESIC_OF_COHERENCE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"GEODESIC_OF_COHERENCE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="GEODESIC_OF_COHERENCE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"GEODESIC_OF_COHERENCE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.geodesic_of_coherence import GeodesicOfCoherence\n>>> # TODO: fill example"
