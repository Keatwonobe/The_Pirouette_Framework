"""
Auto-generated from dictionary entry: SCALE_INVARIANT_GEODESIC
...
class ScaleInvariantGeodesic(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="SCALE_INVARIANT_GEODESIC:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"SCALE_INVARIANT_GEODESIC:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="SCALE_INVARIANT_GEODESIC:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"SCALE_INVARIANT_GEODESIC:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.scale_invariant_geodesic import ScaleInvariantGeodesic\n>>> # TODO: fill example"
