"""
Auto-generated from dictionary entry: PHYSICALLY_LOAD_BEARING
...
class PhysicallyLoadBearing(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="PHYSICALLY_LOAD_BEARING:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"PHYSICALLY_LOAD_BEARING:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="PHYSICALLY_LOAD_BEARING:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"PHYSICALLY_LOAD_BEARING:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.physically_load_bearing import PhysicallyLoadBearing\n>>> # TODO: fill example"
