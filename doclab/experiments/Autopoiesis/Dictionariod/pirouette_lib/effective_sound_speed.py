"""
Auto-generated from dictionary entry: EFFECTIVE_SOUND_SPEED
...
class EffectiveSoundSpeed(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="EFFECTIVE_SOUND_SPEED:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"EFFECTIVE_SOUND_SPEED:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="EFFECTIVE_SOUND_SPEED:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"EFFECTIVE_SOUND_SPEED:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.effective_sound_speed import EffectiveSoundSpeed\n>>> # TODO: fill example"
