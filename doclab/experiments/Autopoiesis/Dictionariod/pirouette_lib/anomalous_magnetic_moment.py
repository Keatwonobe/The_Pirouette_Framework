"""
Auto-generated from dictionary entry: ANOMALOUS_MAGNETIC_MOMENT
...
class AnomalousMagneticMoment(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="ANOMALOUS_MAGNETIC_MOMENT:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"ANOMALOUS_MAGNETIC_MOMENT:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="ANOMALOUS_MAGNETIC_MOMENT:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"ANOMALOUS_MAGNETIC_MOMENT:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.anomalous_magnetic_moment import AnomalousMagneticMoment\n>>> # TODO: fill example"
