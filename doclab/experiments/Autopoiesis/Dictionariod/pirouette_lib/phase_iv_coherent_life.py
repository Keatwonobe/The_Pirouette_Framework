"""
Auto-generated from dictionary entry: PHASE_IV_COHERENT_LIFE
...
class PhaseIvCoherentLife(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="PHASE_IV_COHERENT_LIFE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"PHASE_IV_COHERENT_LIFE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="PHASE_IV_COHERENT_LIFE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"PHASE_IV_COHERENT_LIFE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.phase_iv_coherent_life import PhaseIvCoherentLife\n>>> # TODO: fill example"
