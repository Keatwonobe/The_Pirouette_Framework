"""
Auto-generated from dictionary entry: CRUCIBLE_OF_COHERENCE
...
class CrucibleOfCoherence(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="CRUCIBLE_OF_COHERENCE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"CRUCIBLE_OF_COHERENCE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="CRUCIBLE_OF_COHERENCE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"CRUCIBLE_OF_COHERENCE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.crucible_of_coherence import CrucibleOfCoherence\n>>> # TODO: fill example"
