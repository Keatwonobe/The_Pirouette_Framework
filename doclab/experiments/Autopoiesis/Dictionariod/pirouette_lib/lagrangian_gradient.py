"""
Auto-generated from dictionary entry: LAGRANGIAN_GRADIENT
...
class LagrangianGradient(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"LAGRANGIAN_GRADIENT:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="LAGRANGIAN_GRADIENT:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"LAGRANGIAN_GRADIENT:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.lagrangian_gradient import LagrangianGradient\n>>> # TODO: fill example"
