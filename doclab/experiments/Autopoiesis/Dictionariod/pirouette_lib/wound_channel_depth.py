"""
Auto-generated from dictionary entry: WOUND_CHANNEL_DEPTH
...
Auto-generated at 2025-10-19T01:26:51.604039Z
"""
...
class WoundChannelDepth(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"WOUND_CHANNEL_DEPTH:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="WOUND_CHANNEL_DEPTH:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"WOUND_CHANNEL_DEPTH:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.wound_channel_depth import WoundChannelDepth\n>>> # TODO: fill example"
