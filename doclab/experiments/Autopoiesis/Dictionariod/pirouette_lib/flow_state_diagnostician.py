"""
Auto-generated from dictionary entry: FLOW_STATE_DIAGNOSTICIAN
...
class FlowStateDiagnostician(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="FLOW_STATE_DIAGNOSTICIAN:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"FLOW_STATE_DIAGNOSTICIAN:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="FLOW_STATE_DIAGNOSTICIAN:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"FLOW_STATE_DIAGNOSTICIAN:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.flow_state_diagnostician import FlowStateDiagnostician\n>>> # TODO: fill example"
