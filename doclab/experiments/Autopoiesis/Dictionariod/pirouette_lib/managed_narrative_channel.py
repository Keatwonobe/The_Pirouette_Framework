"""
Auto-generated from dictionary entry: MANAGED_NARRATIVE_CHANNEL
...
class ManagedNarrativeChannel(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="MANAGED_NARRATIVE_CHANNEL:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"MANAGED_NARRATIVE_CHANNEL:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="MANAGED_NARRATIVE_CHANNEL:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"MANAGED_NARRATIVE_CHANNEL:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.managed_narrative_channel import ManagedNarrativeChannel\n>>> # TODO: fill example"
