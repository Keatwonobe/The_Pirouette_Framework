"""
Auto-generated from dictionary entry: GEOMETRIC_CAUSALITY
...
class GeometricCausality(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"GEOMETRIC_CAUSALITY:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="GEOMETRIC_CAUSALITY:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"GEOMETRIC_CAUSALITY:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.geometric_causality import GeometricCausality\n>>> # TODO: fill example"
