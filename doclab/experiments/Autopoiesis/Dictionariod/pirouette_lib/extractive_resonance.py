"""
Auto-generated from dictionary entry: EXTRACTIVE_RESONANCE
...
class ExtractiveResonance(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="EXTRACTIVE_RESONANCE:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"EXTRACTIVE_RESONANCE:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="EXTRACTIVE_RESONANCE:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"EXTRACTIVE_RESONANCE:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.extractive_resonance import ExtractiveResonance\n>>> # TODO: fill example"
