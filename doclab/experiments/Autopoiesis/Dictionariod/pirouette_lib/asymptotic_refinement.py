"""
Auto-generated from dictionary entry: ASYMPTOTIC_REFINEMENT
...
class AsymptoticRefinement(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="ASYMPTOTIC_REFINEMENT:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"ASYMPTOTIC_REFINEMENT:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="ASYMPTOTIC_REFINEMENT:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"ASYMPTOTIC_REFINEMENT:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.asymptotic_refinement import AsymptoticRefinement\n>>> # TODO: fill example"
