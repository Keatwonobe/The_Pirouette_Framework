"""
Auto-generated from dictionary entry: PRINCIPLE_OF_HARMONIC_INCLUSION
...
class PrincipleOfHarmonicInclusion(Term):
    ...
    def measure(self, ctx: Context) -> List[Measurement]:
        ...
        tasks.append(Measurement(
            name="PRINCIPLE_OF_HARMONIC_INCLUSION:noop",
        ...
        for i, desc in enumerate(MEASURE_BULLETS, 1):
            tasks.append(Measurement(
                name=f"PRINCIPLE_OF_HARMONIC_INCLUSION:m{i}",
                description=desc,
                compute=lambda c: c.get("data")
            ))
        return tasks
    ...
    def constraints(self) -> List[Constraint]:
        ...
        checks.append(Constraint(
            name="PRINCIPLE_OF_HARMONIC_INCLUSION:exists",
        ...
        for i, desc in enumerate(CONSTRAINT_BULLETS, 1):
            checks.append(Constraint(
                name=f"PRINCIPLE_OF_HARMONIC_INCLUSION:c{i}",
                description=desc,
                check=_always_true
            ))
        return checks
    ...
    def example(self) -> str:
        ...
        """
        return ">>> from pirouette_lib.principle_of_harmonic_inclusion import PrincipleOfHarmonicInclusion\n>>> # TODO: fill example"
