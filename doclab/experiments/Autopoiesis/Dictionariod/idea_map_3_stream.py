...
import re, json, argparse, math, time, os, sqlite3
...
def _token_entropy(counter: Counter) -> float:
    """Shannon entropy normalized to [0,1] over token frequency."""
    if not counter: return 0.0
    total = sum(counter.values())
    if total <= 0: return 0.0
    import math
    ps = [c/total for c in counter.values() if c>0]
    H = -sum(p*math.log(p+1e-12) for p in ps)
    Hmax = math.log(len(ps)+1e-12)
    return 0.0 if Hmax<=0 else max(0.0, min(1.0, H/Hmax))


def _lexical_dimension(node: dict) -> float:
    # Map normalized entropy [0,1] -> fractal-like dimension in [1,2]
    ent = _token_entropy(node.get("tokens", Counter()))
    return 1.0 + ent  # 1..2

def _node_coherence(node: dict) -> float:

    # Lower residue -> higher coherence; peaky token distribution -> higher coherence; aliases/symbol add mild boost
    residue = float(node.get("residue", 0.0))
    ent = _token_entropy(node.get("tokens", Counter()))
    alias_boost = 0.02 * len(node.get("aliases", []))
    sym_boost   = 0.08 if (node.get("symbol") or "").strip() else 0.0
    raw = (1.0 - min(1.0, residue)) * (1.0 - 0.5*ent) + alias_boost + sym_boost
    # squash to [0,1]
    return max(0.0, min(1.0, 1.0/(1.0 + math.exp(-3.0*(raw-0.5)))))
...
def build_graph(dict_dir: Path, out_dir: Path):
    ...
    # --- Streaming pair generation with disk-backed dedupe and tunable thresholds ---
    MAX_DF_FRAC = float(os.getenv("IDEA_DF_FRAC", "0.03"))
    MIN_COMMON  = int(os.getenv("IDEA_MIN_COMMON", "5"))
    PER_TOKEN_CAP = int(os.getenv("IDEA_PER_TOKEN_CAP", "200"))
    COH_EDGE_WEIGHT = float(os.getenv("IDEA_COH_WEIGHT", "0.05"))  # add to edge from node coherence
    FRAC_EDGE_WEIGHT = float(os.getenv("IDEA_FRAC_WEIGHT", "0.05"))  # add via lexical dimension similarity

    # Build token document frequencies
    doc_freq = Counter()
    for node in nodes.values():
        doc_freq.update(node["tokens"].keys())

    max_df = max(2, int(len(nodes) * MAX_DF_FRAC))
    rare_tokens = {t for t, count in doc_freq.items() if 1 < count <= max_df}
    ...
    # Disk-backed pair deduplication
    dedupe_db = out_dir / "pairs.sqlite"
    conn = sqlite3.connect(dedupe_db)
    conn.execute("PRAGMA journal_mode=WAL;")
    conn.execute("CREATE TABLE IF NOT EXISTS seen (u TEXT, v TEXT, PRIMARY KEY(u,v));")

    def pairs_stream():
        for tok, docs in inverted_index.items():
            docs = sorted(docs)[:PER_TOKEN_CAP]
            for i in range(len(docs)):
                ...
                for j in range(i+1, len(docs)):
                    ...
                    try:
                        conn.execute("INSERT OR IGNORE INTO seen(u,v) VALUES(?,?)", (u,v))
    ...
    processed_pairs = 0
    for a, b in pairs_stream():
        if a not in node_keys or b not in node_keys:
            continue

        s = rare_token_overlap(nodes[a]["tokens"], nodes[b]["tokens"], min_common=MIN_COMMON)
        if s > 0:
            w = min(0.10, 0.02 * s)

            # --- Add coherence-based bonus layer (Pirouette metric) ---
            try:
                ca = _node_coherence(nodes[a]); cb = _node_coherence(nodes[b])
            except Exception:
                ca = cb = 0.0
            w += COH_EDGE_WEIGHT * ( (ca + cb) * 0.5 )

            # Fractal-dimension-like similarity bonus
            try:
                da = _lexical_dimension(nodes[a]); db = _lexical_dimension(nodes[b])
                sim = 1.0 - min(1.0, abs(da - db))  # 0..1
            except Exception:
                sim = 0.0
            w += FRAC_EDGE_WEIGHT * sim
            ...
            if current_time - last_checkpoint_time > 10 or i % 1000 == 999:
                print(f"  ...processed {i + 1}/{processed_pairs} pairs. Checkpointing...")
    ...
    serializable_edges = {f"{u},{v}": w for u, inner in edges.items() for v, w in inner.items()}
    final_data = {"progress_index": processed_pairs, "edges": serializable_edges}
    try:
        with checkpoint_temp_file.open("w", encoding="utf-8") as f:
