...
class HarmonicResonanceFinder:
    """
    Stochastically identifies natural resonant ratios in the landscape
    by comparing local maxima against each other.
    """
    ...
    def explore_natural_ratios(self, bounds_m, bounds_l, min_ratio=1.1, max_ratio=5.0):
        """
        1. Finds all local maxima in the potential.
        2. Computes ratios between all pairs of maxima.
        3. Returns clusters of ratios found.
        """
        logger.info("Scanning for local maxima to determine natural ratios...")
        
        # 1. Find all distinct peaks in the landscape
        # We increase n_starts to ensure we catch the whole topology
        peaks = self.potential.find_local_maxima(bounds_m, bounds_l, n_starts=50)
        
        if len(peaks) < 2:
            logger.warning("Not enough peaks found to calculate ratios.")
            return []

        logger.info(f"Found {len(peaks)} distinct coherence peaks.")
        for i, (m, l, xi) in enumerate(peaks):
            logger.info(f"  Peak {i+1}: ξ={xi:.4f} at (m²={m:.3f}, λ={l:.3f})")

        # 2. Calculate pairwise ratios
        found_ratios = []
        
        # Compare every peak against every other peak (assuming Peak A > Peak B)
        for i in range(len(peaks)):
            for j in range(len(peaks)):
                if i == j: continue
                
                xi_numerator = peaks[i][2]
                xi_denominator = peaks[j][2]
                
                ratio = xi_numerator / xi_denominator
                
                # Filter for physically relevant ratios
        ...
        # 3. Sort by ratio for analysis
        found_ratios.sort(key=lambda x: x['ratio'])
        return found_ratios

    def analyze_ratio_clusters(self, ratios, tolerance=0.05):
        """
        Groups similar ratios to find the 'dominant' harmonics.
        """
        if not ratios:
            return []
            
        clusters = []
        current_cluster = [ratios[0]]
        
        for r in ratios[1:]:
            # If this ratio is within tolerance of the current cluster's average
            avg_val = sum(x['ratio'] for x in current_cluster) / len(current_cluster)
            
            if abs(r['ratio'] - avg_val) < tolerance:
        ...
        summary = []
        for c in clusters:
            avg_ratio = sum(x['ratio'] for x in c) / len(c)
            strength = len(c) # How many pairs produced this ratio
            summary.append({
                'ratio': avg_ratio,
                'strength': strength,
                'examples': c[:3] # Keep top 3 examples
            })
            
        # Sort by strength (frequency of occurrence)
        return sorted(summary, key=lambda x: x['strength'], reverse=True)

class MonteCarloScanner:
    """
    Wraps the CoherenceScanner to run multiple distinct 'epochs'.
    Aggregates results to filter out phase jitter and find the
    true invariant ratios.
    """
    def __init__(self, m_range, l_range, simulator_func):
        self.m_range = m_range
        self.l_range = l_range
    ...
    def run_ensemble(self, n_epochs=10):
        logger.info(f"\n{'='*70}")
        logger.info(f"MONTE CARLO ENSEMBLE SCAN ({n_epochs} Epochs)")
        logger.info(f"{'='*70}")
        
        all_ratios = []
        
        # 1. Run multiple independent scans
        for i in range(n_epochs):
            print(f"  > Running Epoch {i+1}/{n_epochs}...")
            
            # Create a fresh scanner for each epoch to ensure independence
            scanner = CoherenceScanner(self.m_range, self.l_range, self.simulator_func)
            
            # We bypass the full run() to access internal components directly
            # Phase 1: Sample
            from scipy.stats import qmc
            sampler = qmc.LatinHypercube(d=2)
            sample = sampler.random(n=30) # Slightly lighter sampling per epoch
            m_samples = sample[:, 0] * (self.m_range[1] - self.m_range[0]) + self.m_range[0]
            l_samples = sample[:, 1] * (self.l_range[1] - self.l_range[0]) + self.l_range[0]
            ...
            # Phase 2: Flow (Quick)
            top_samples = sorted(scanner.potential.samples, key=lambda x: x[2], reverse=True)[:5]
            for m, l, _ in top_samples:
                traj = CoherenceFlowTrajectory(m, l, scanner.potential, self.simulator_func)
                traj.converge(self.m_range, self.l_range, max_steps=15)

            # Phase 3: Harvest Ratios
            ratios = scanner.harmonic_finder.explore_natural_ratios(
                self.m_range, self.l_range
            )
            all_ratios.extend(ratios)
            
        # 2. Statistical Analysis of the Ensemble
        logger.info(f"\nProcessing {len(all_ratios)} total candidate ratios...")
        
        # Use a slightly wider tolerance to catch the "drifting" 2.35 vs 2.39
        # This groups them into a single robust cluster.
        finder = HarmonicResonanceFinder(None, None) # Helper instance
        clusters = finder.analyze_ratio_clusters(all_ratios, tolerance=0.1)
        
        # 3. Output Final Stabilized Table
        print("\n" + "="*60)
        print(f"{'STABILIZED RATIO':<18} | {'CONFIDENCE':<12} | {'PHYSICS NOTE'}")
        print("="*60)
        
        results = []
        for c in clusters:
            ratio = c['ratio']
            confidence = c['strength'] # How many times it appeared across ALL epochs
            
            # Filter out noise (must appear in at least 20% of epochs to count)
            if confidence < n_epochs * 0.2:
                continue
                
            note = ""
            if abs(ratio - 1.79) < 0.1: note = "SU(2)/SU(3) Candidate"
            if abs(ratio - 2.31) < 0.1: note = "U(1)/SU(3) Candidate"
            if abs(ratio - 1.5) < 0.1:  note = "3/2 Harmonic"
            if abs(ratio - 1.618) < 0.05: note = "Phi (Golden Ratio)"
            
            # Calculate variance within the cluster to show stability
            r_vals = [x['ratio'] for x in c['examples']] # heuristic
            variance = np.std(r_vals) if len(r_vals) > 1 else 0.0
            
            print(f"{ratio:<18.4f} | {confidence:<12} | {note}")
            results.append((ratio, confidence, note))
...
if __name__ == "__main__":
    # Example usage with mock simulator
    def mock_simulator(m_sq, lambda_4):
        """Mock simulator for testing (replace with real PirouetteLattice)"""
        # Synthetic landscape with a peak and harmonics
        x = (m_sq + 1.0) / 2.0
        y = (lambda_4 - 2.0) / 3.0
    ...
    mc_scanner.run_ensemble(n_epochs=10)