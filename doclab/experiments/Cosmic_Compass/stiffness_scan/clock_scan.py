...
class TemporalClockAnalyzer:
    """
    Analyzes the 'Heartbeat' of the Pirouette by looking for 
    time-domain periodicity (Autocorrelation) rather than frequency.
    
    This detects 'Clocks' and 'Pulses' even in non-sinusoidal signals.
    """
    
    def find_internal_clock(self, dataframe):
        logger.info(f"\n{'='*60}")
        logger.info("TEMPORAL CLOCK SEARCH (Autocorrelation)")
        logger.info(f"{'='*60}")
        
        # 1. Prepare the Signal (The Ratio)
        # We work with the Ratio because it cancels out global jitter
        signal = dataframe['ratio'].values
        ...
        # correlates signal with itself at all possible delays (lags)
        n = len(signal)
        acorr = np.correlate(signal, signal, mode='full')
        acorr = acorr[n-1:] / n  # Keep positive lags only, normalize
        
        # 3. Find the "Beat" (Peaks in Autocorrelation)
        # We skip lag=0 (which is always 1.0)
        peaks, properties = find_peaks(acorr, height=0.1, distance=5)
        
        # Filter out the lag=0 peak
        peaks = [p for p in peaks if p > 5]
        
        if not peaks:
            logger.warning("No consistent clock found. The system might be purely chaotic.")
            return
            
        # 4. Analyze the Primary Clock Cycle
        primary_lag = peaks[0] # The first time the pattern repeats
        correlation_strength = acorr[primary_lag]
        
        logger.info(f"PRIMARY CLOCK FOUND at Lag: {primary_lag} steps")
        logger.info(f"Clock Stability (Correlation): {correlation_strength:.4f}")
        
        # Check for harmonics (Is there a peak at 2*Lag, 3*Lag?)
        secondary_lag = peaks[1] if len(peaks) > 1 else None
        if secondary_lag:
            ratio = secondary_lag / primary_lag
            logger.info(f"Secondary Echo at {secondary_lag} steps (Ratio: {ratio:.2f}x)")
            if abs(ratio - 2.0) < 0.2:
                logger.info(" >> Confirmed Harmonic Clockwork (1-2 Rhythm)")

        # 5. Visual Output
        self._plot_autocorrelation(acorr, peaks)
        
    def _plot_autocorrelation(self, acorr, peaks):
        plt.figure(figsize=(10, 6))
        
        lags = np.arange(len(acorr))
        
        # Plot the correlation curve
        plt.plot(lags, acorr, 'b-', linewidth=1.5, label='Autocorrelation')
        plt.fill_between(lags, acorr, 0, where=(acorr>0), color='blue', alpha=0.1)
        
        # Mark the Clock Ticks
        for p in peaks[:5]: # Show first 5 beats
            plt.plot(p, acorr[p], 'ro')
            plt.text(p, acorr[p] + 0.05, f"{p}", ha='center', color='red', fontweight='bold')
            
        plt.axhline(y=0, color='k', linestyle='-', alpha=0.3)
        plt.axhline(y=1.0/np.e, color='g', linestyle='--', alpha=0.5, label='Decay Threshold')
        
        plt.title("System Memory & Internal Clock")
        plt.xlabel("Time Lag (Steps)")
        plt.ylabel("Correlation Strength")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.xlim(0, len(acorr)//2) # Zoom in on the relevant first half
        
        plt.savefig('clock_analysis.png')
        logger.info("[Saved clock plot to 'clock_analysis.png']")
...
if __name__ == "__main__":
    ...
    df_results = scanner.run_diagnostic(steps=1500) # Need more steps for FFT
    
    # 3. Run Spectral Analysis
    analyzer = SpectralBreathingAnalyzer()
    analyzer.analyze_spectrum(df_results)
    
    # 2. Run the Clock Analyzer
    clock = TemporalClockAnalyzer()
    clock.find_internal_clock(df_results)
    pass