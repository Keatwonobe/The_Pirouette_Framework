...
# --- CONFIGURATION ---
# Increase this to make the central spire taller (relative to the landscape)
SPIRE_AMP = 6.0  
# Increase this to stretch the entire model vertically
GLOBAL_HEIGHT_SCALE = 8.0 
# ---------------------

def minimal_memory_ply_export(input_file="phase_space_raw.npz", output_file="pirouette_deep_field_2.ply"):
    """
    ULTRA-DEFENSIVE PLY writer with Z-AXIS UNLEASHED.
    """
    ...
    with np.load(input_file) as npz_data:
        ...
        # Calculate the natural height of the "hills" (non-trapped points)
        NATURAL_MAX = float(np.max(Z_temp[~trapped_mask]) if np.any(~trapped_mask) else 1.0)
        
        # HERE IS THE FIX: We multiply the natural max by our new SPIRE_AMP
        MONOLITH_HEIGHT = NATURAL_MAX * SPIRE_AMP
        
        Z_temp[trapped_mask] = MONOLITH_HEIGHT
        
        # Normalize so the highest point hits our GLOBAL_HEIGHT_SCALE
        Z_SCALE = GLOBAL_HEIGHT_SCALE / float(np.max(Z_temp))
    ...
    logger.info(f"      Natural Hill Height: {NATURAL_MAX:.3f}")
    logger.info(f"      New Spire Height: {MONOLITH_HEIGHT:.3f}")
    ...
    if not vertices_complete:
        ...
        try:
            for chunk_idx in range(chunk_start_idx, total_chunks):
                ...
                # RE-CALCULATE Z WITH NEW HEIGHTS
