...
def main():
    ...
    config = {
        ...
        'results_csv': "vr_analysis_results_from_tar_2.csv",
        'max_files_to_process': 100 # <--- SET THIS to a number, or None to run all files
    }
    ...
    if os.path.exists(config['archive_path']):
        ...
        print(f"[PREP] ✓ Full manifest created with {full_manifest_count} text files.")

        # --- NEW: Limit and shuffle the manifest if max_files_to_process is set ---
        if config['max_files_to_process'] is not None:
            limit = config['max_files_to_process']
            print(f"--- Limiting run to a random sample of {limit} files out of {full_manifest_count} total. ---")
    ...
    try:
        while True:
            ...
            print(f"\n--- Main Loop --- Target: {len(manifest)} files --- Completed: {len(completed_files)}/{len(manifest)}")

            if len(completed_files) >= len(manifest):
                print("\n--- ✅ All targeted files have been processed or attempted! ---")
                break
            ...
            for item in manifest:
                ...
                while not result_queue.empty():
                    ...
                    if res['status'] == 'complete':
                        ...
                        # Log final result
                        # Ensure all possible keys are present for consistent CSV columns
                        final_res = {
                            "file": res.get("file"), "category": res.get("category"), 
                            "velcridance_score": res.get("velcridance_score", 0), 
                            "radiance_score": res.get("radiance_score", 0), 
                            "kurtosis_score": res.get("kurtosis_score", 0), 
                            "drift_score": res.get("drift_score", 0),
                            "anisotropy_score": res.get("anisotropy_score", 0), 
            ...
            if not found_work:
                print("\n--- No new work found in the targeted set. Shutting down. ---")
                break
