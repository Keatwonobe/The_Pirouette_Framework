...
class PirouetteWorld:
    ...
    def step(self, dt=0.1):
        ...
        self.biosphere.ext_Ta_ddot += 0.1 * self.innovation.Gamma  # Tech healing

        # INST — Coherence Warfare & Collapse
        ...
        # INNO — Growth / Fragility of Innovation
        self.innovation.ext_Gamma_ddot += 0.001 * human_gamma_total
        self.innovation.ext_Ta_ddot -= 0.1 * (1.0 - self.innovation.Ta)

        # WEAVER — Stabilization input
        self.biosphere.ext_Ta_ddot += 0.05 * self.coherent_weaver.Ki
        self.innovation.ext_Ta_ddot += 0.05 * self.coherent_weaver.Ki

        # --- SHOCKS — Event Triggers ---
        if self.year > 2026.0 and self.year < 2026.2:
            if "SCOTUS Shock" not in self.event_log:
                self.event_log.append(f"{self.year:.1f}: SCOTUS Shock - Constitutional Crisis")
            self.usa.ext_Ta_ddot -= 0.5 # Apply a sharp, negative force to USA's cohesion

        if (self.usa.Ta < 0.6 and self.usa.Gamma > self.bloc.Gamma and
            not any("Kinetic War" in e for e in self.event_log)):
            phase_opposition = (1 - np.cos(self.usa.Phi - self.bloc.Phi)) / 2
...
def plot_results(history_df):
    ...
    fig.suptitle('PirouetteWorld Simulation Results', fontsize=18)

    # Plot 1: Time-Adherence (Ta)
    ...
    # Plot 2: Rigidity / Stored Energy (Gamma)
    ...
    plt.savefig('pirouette_simulation_shock_scenario.png')
    print("\nGenerated plot: pirouette_simulation_shock_scenario.png")

# ===========================
# Fleshed-Out Test Case
# ===========================
if __name__ == "__main__":
    print("Running PirouetteWorld simulation with SCOTUS Shock Scenario...")