...
class EngineLensSolver:
    ...
    def choose_move(self, board: chess.Board):
        """
        Let the engine pick the move, then compute the engram & residue
        for that chosen move only.

        IMPORTANT: This function must NOT mutate the shared board. It works
        on copies and returns a legal move plus metadata.
        """
        ...
        # ---- 1) Ask engine for a move on a COPY of the board ----
        analysis_board = board.copy()
        info = self.engine.analyse(analysis_board, chess.engine.Limit(depth=self.depth))
        pv = info.get("pv")
        if pv and len(pv) > 0:
            engine_move = pv[0]
        else:
        ...
        # Ensure the move is actually legal in *this* board state
        if engine_move not in legal_moves:
            # Fallback: trust legality first
            engine_move = legal_moves[0]

        # ---- 2) Build engram on copies only ----
        ...
        engram = build_action_engram(
            board_before, board_after, engine_move.uci(), self.cfg, D_pack
        )

        # Add engine score (from WHITE's POV for consistency)
        ...
        # NOTE: we do NOT push on the real board here.
        # The caller (run_games) is responsible for `adapter.push(board, move)`.
        return engine_move, meta
...
def run_games(num_games: int, out_dir: str, cfg: dict):
    ...
    with chess.engine.SimpleEngine.popen_uci(engine_path) as engine:
        ...
        for g in range(num_games):
            ...
            print(f"\n=== Game {g+1}/{num_games} (ENGINE+LENS vs GREEDY) ===")
