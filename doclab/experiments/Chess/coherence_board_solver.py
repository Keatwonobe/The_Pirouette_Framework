...
def _safe_all_squares():
    # <--- FIX 1: This is the fix for the TypeError and state corruption.
    # The chess.SQUARES global was being corrupted by re-entrancy bugs.
    # We remove the dependency and return a simple, stable range.
    return range(64)
...
def compute_dark_residue_chess(board_before: chess.Board,
    ...
    - Div(welfare): our king safety + our structure got worse
    - Ext(risk): opponent's attack surface increased
    - AttentionDebt: number of own undefended pieces increased
    - LossAutonomy: our next-move branching got smaller
    """
    α = cfg["D_alpha"]
    β = cfg["D_beta"]
    γ = cfg["D_gamma"]
    ...
    try:
        ...
        # count our next legal moves (simulate: it's our turn now)
        ...
        # This can fail if the board is in a weird state (e.g., no king)
        # print(f"Error in Dark Residue calculation: {e}")
        return {
...
class CoherenceAsymmetrySolver:
    ...
    def _peak_coherence_one_ply(self, board):
        # This function receives a board, which is a COPY
        # and it is safe to mutate.
    ...
    def choose_move(self, board, log_masks=False):
        ...
        for mv in legal:
            ...
            # <--- FIX 2: This is the "Silent Fail" fix (performance).
            # We implement the "laser pointer" prune. If a move is not
            # from one of our top actors, we don't evaluate it.
            ...
            # my branch
            adapter.push(board, mv)
            
            # <--- FIX 3: This is the "Corruption" fix (stability).
            # We pass board.copy() to the sub-routine. This isolates
            # its board mutations and prevents it from corrupting the
            # main 'board' object this loop is iterating on.
            my_future = self._peak_coherence_one_ply(board.copy())

            # opponent branch
            try:
                adapter.null_move(board)
                # Pass a copy here as well for safety.
...
def run_games(num_games: int, out_dir: str, cfg: dict):
    ...
    for g in range(num_games):
        ...
        for ply in range(cfg["max_plies"]):
            ...
            print(f"  Ply {ply} ({side}): {move_s:6} (Score: {meta.get('asym_score', 0):.2f}, D: {meta.get('dark_residue', {}).get('D', 0):.2f})")
