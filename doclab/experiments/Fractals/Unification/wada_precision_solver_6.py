...
# --- N-D TRANSLATION (FIXED: TOP-DOWN) ---

@njit(fastmath=True)
def nd_translate(center_nd, offset_s):
    """
    Translates an N-D center by a float64 scalar offset.
    Iterates Top-Down (0 to N) to apply the offset to the significant digits first.
    """
    N = len(center_nd)
    result_nd = np.empty_like(center_nd) 
    
    remainder = offset_s 
    
    # Iterate from Largest Component to Smallest (Top-Down)
    for i in range(N):
...
# ==============================================================================
# II. HYPER-OPTIMIZED SCALAR PHYSICS KERNEL
# ==============================================================================

@njit(fastmath=True)
def get_basin_single_dd_optimized(m_in_dd, l_in_dd, t_max=60.0, dt=0.05, escape_r2=16.0):
    """
    Scalar-based DD physics kernel. 
    Significantly faster than array-based version by avoiding allocation.
    """
    # Unpack to scalars
...
def get_drift_and_span_nd(res, zoom, center_x_nd, center_y_nd):
    """
    Calculates drift (Center of Mass) and Span (Bounding Box).
    """
    ...
    # 1. MAP GENERATION (Run the Optimized DD Kernel)
    for y in prange(res):
        for x in range(res):
            ...
            # Pass first two elements (DD) to optimized kernel
            m_dd_in = abs_x_nd[:2]
            l_dd_in = abs_y_nd[:2]
            ...
            if basin != 0: 
                # Reconstruct for rotational symmetry check (low precision is fine here)
...
# ==============================================================================
# IV. HYPER-OPTIMIZED SEEKER (WITH FORCED DIVE)
# ==============================================================================
...
def seek_absolute_tip_hyper_optimized(start_zoom, start_cx, start_cy, N_parts, 
    ...
    for i in range(max_iterations):
        ...
        # --- ZOOM UPDATE (FORCED DIVE) ---
        # Factor 0.4 ensures that even if the structure fills the screen (span=width),
        # we will still zoom in (multiply by ~0.8) rather than zoom out.
        target_zoom = span * 0.4
        
        # Ratchet: Never grow larger.
        potential_new_zoom = max(target_zoom, curr_zoom * 0.1) # Limit drop speed to 10x
        curr_zoom = min(curr_zoom, potential_new_zoom)
        
        if curr_zoom < 1e-60: break
