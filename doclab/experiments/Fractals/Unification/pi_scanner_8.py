...
M_MIN, M_MAX = -240000000, 240000000
L_MIN, L_MAX = -240000000, 240000000
RES_BASE = 1000      # Grid resolution for all analyses
MAX_STEPS = 500      # Higher steps = deeper fractal/chromatic detail
...
def get_force_weights(m, lam):
    ...
    scale = np.sqrt(mag) # Gold force scaling
...
def compute_helicity_grid(m_vals, l_vals):
    ...
    print(f"[A1: HELICITY] Finished in {time.time()-start:.2f}s")
    return H
...
# --- ANALYSIS 2 CORE: CHROMATIC FIELD (MODIFIED for Escape Mask) ---
...
def render_chromatic_scan(m_vals, l_vals):
    ...
    escape_mask = np.zeros((h, w), dtype=np.bool_) # MODIFIED: New mask
    ...
    for i in range(h):
        for j in range(w):
            ...
            escape_mask[i, j] = not escaped # MODIFIED: True if trapped (in basin)
            
    return image, escape_mask # MODIFIED: Return the mask
...
def run_microscope_zoom(m_knot, l_knot, zoom_radius=12):
    ...
    """
    RES = 500 # Use lower resolution for the zoom for speed, or keep 1000 if high detail is needed
    ...
    img_core_raw, _ = render_chromatic_scan(m_vals, l_vals) # We ignore the mask here
    
    print(f"[A3: ZOOM] Finished scan in {time.time()-start:.2f}s")
...
def run_unified_analysis(m_min=M_MIN, m_max=M_MAX, l_min=L_MIN, l_max=L_MAX, res=RES_BASE):
    ...
    # Explicitly compile the new trace_chromatic_pixel signature
    ...
    ## 1. Stability and Geometry Analysis (Helicity-based) ##
    ...
    # Basin Mask 1 (Helicity-based: the 'modern' approach)
    basin_mask_h = H <= np.quantile(H, 0.25) 
    eroded_h = binary_erosion(basin_mask_h)
    boundary_mask_h = basin_mask_h & ~eroded_h

    # Fractal Dimension 1
    D_f_h = box_counting_dimension(boundary_mask_h)
    ...
    ## 2. Chromatic Field Analysis & Escape-Time Mask (MODIFIED) ##
    
    # MODIFIED: raw_img now also returns the escape_mask
    raw_img, escape_mask = render_chromatic_scan(m_vals, l_vals)
    
    # Basin Mask 2 (Escape-Time based: the 'original' approach)
    # The mask is already True if trapped: `escape_mask`
    eroded_e = binary_erosion(escape_mask)
    boundary_mask_e = escape_mask & ~eroded_e
    
    # Fractal Dimension 2
    D_f_e = box_counting_dimension(boundary_mask_e)
    ...
    print(f"\n--- 1 & 2. Stability & Geometry & Chromatic Analysis Results ---")
    print(f"**Max Helicity (Knot Core):** H_max={H_max:.4f}")
    print(f"**Knot Coordinates (m, λ):** ({m_knot:.4f}, {l_knot:.4f})")
    
    print(f"\n**[Fractal Dimension 1: Helicity-based (Modern/V3)]**")
    print(f"  Box Counting $D_f$ (H-based): {D_f_h:.4f}")
    
    print(f"\n**[Fractal Dimension 2: Escape-Time based (Original Math)]**")
    print(f"  Box Counting $D_f$ (E-based): {D_f_e:.4f}")
    if D_f_e > 1.0:
        print(f"  Interpretation: Both methods suggest a **Fractal boundary** (Wada boundary analog), indicating **quantum chaos**.")
    print("-" * 40)
