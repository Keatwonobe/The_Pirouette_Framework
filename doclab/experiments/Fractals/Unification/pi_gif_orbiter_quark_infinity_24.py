...
# --- CONFIGURATION ---
OUTPUT_FILENAME = "proton_phosphor_highgain.gif"
FRAMES = 160            
GLOBAL_SCALE = 28.0     

# VISUAL SETTINGS
SUBSTRATE_GAIN = 0.5    # <--- NEW: Buffs the background signal (3.0 = 300%)
                        #      Increase this to see more "subquark" ripples.
...
def run_phosphor_engine():
    print("--- ☢️ INITIATING PHOSPHOR ENGINE (HIGH GAIN) ---")
    ...
    print(f"Rendering frames with Substrate Gain: {SUBSTRATE_GAIN}x ...")
    for f in range(FRAMES):
        ...
        for t in trackers:
            ...
            paint_trail_blob(phosphor_layer, px, py, blob_size, paint_c, 1.0) 
        ...
        # --- APPLY SUBSTRATE GAIN ---
        norm_map *= SUBSTRATE_GAIN
        norm_map = np.clip(norm_map, 0.0, 1.0)
        # ----------------------------

        map_rgb = (plt.get_cmap('inferno')(norm_map)[:, :, :3] * 255) 
