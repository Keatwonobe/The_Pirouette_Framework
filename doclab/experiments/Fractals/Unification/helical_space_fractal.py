...
class SpaceHelicityScanner:
    """
    THE SPACE HELICITY SCANNER
    Measures Rotational Sensitivity (Differential Helicity) of the complex
    Space Fractal field.
    """
    def __init__(self, resolution=400):
        ...
        self.M_MIN, self.M_MAX = -2.5, 2.5
        self.L_MIN, self.L_MAX = -2.5, 2.5
    
    def get_force_and_weight(self, m, lam):
        # --- 1. Force Calculation from space_fractal.py ---
        F_teal_m = -(m + 0.866) 
        F_teal_lam = -(lam - 0.5)
        ...
        w_red = np.exp(-(diff_r/80)**2) # This is the crucial drag weight
        ...
        # Return force and the specific drag weight
        return Fm, Flam, nw_red

    def measure_helicity_difference(self, m_start, l_start):
        """
        Runs two particles (Reality and Shadow) through the custom Damped Symplectic
        Integrator to measure maximum angular separation.
        """
        # 1. Reality
        ...
        for t in range(self.max_steps):
            ...
            # Normalized difference (shortest path rotation)
            raw_diff = ang1 - ang2
            ...
            # If the rotation is already maxed out
    ...
    def run_scan(self):
        logger.info(f"\n{'='*60}")
        logger.info(f"SPACE FRACTAL HELICITY PROBE ({self.res}x{self.res})")
        logger.info("Measuring Rotational Sensitivity of the Mixed Field...")
        logger.info(f"{'='*60}")
        
        m_range = np.linspace(self.M_MIN, self.M_MAX, self.res)
        l_range = np.linspace(self.L_MIN, self.L_MAX, self.res)
    ...
    def _plot_map(self, helicity, m_range, l_range):
        logger.info("Generating Differential Helicity Map...")
        
        fig, ax = plt.subplots(figsize=(10, 10), facecolor='#111111')
        
        # Use HSV to visualize angular difference
        im = ax.imshow(helicity, origin='lower', extent=[m_range[0], m_range[-1], l_range[0], l_range[-1]], 
                   cmap='hsv', interpolation='bilinear')
        
        ax.set_title("Space Fractal: Differential κ-Helicity (Rotational Sensitivity)", color='cyan', fontsize=15)
        ax.set_xlabel('Mass Field (m)', color='white')
        ax.set_ylabel('Coupling Field (λ)', color='white')
        ax.tick_params(colors='white')
        
        cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label(r'Sensitivity $(\log |\Delta \theta|_{\max})$', color='white')
        cbar.ax.yaxis.set_tick_params(color='white')
        plt.setp(plt.getp(cbar.ax.axes, 'yticklabels'), color='white')

        plt.tight_layout()
        plt.savefig('space_differential_helicity.png', dpi=150, facecolor='#111111')
        logger.info("Saved analysis to 'space_differential_helicity.png'")


if __name__ == "__main__":
    scanner = SpaceHelicityScanner(resolution=400)
    scanner.run_scan()