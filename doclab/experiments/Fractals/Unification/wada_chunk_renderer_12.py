...
# ==========================================
# 5. MAIN EXECUTION (SCAN DOWN FROM TIP)
# ==========================================
...
MAX_LAYERS = 800
ZOOM_RATE = 1.025      # Rate to zoom OUT (1.025 = 2.5% increase per layer)
# ------------------------------------
...
current_zoom = start_zoom * 2.0 # Back off slightly to ensure we catch the very first pixel

world_x, world_y, world_z = [], [], []
delta_x, delta_y, delta_z = [], [], []
prev_boundaries_mask = None 

print(f"[-] Starting HIGH RES Recording (Zooming OUT from absolute bottom)...")
...
for i in range(MAX_LAYERS):
    ...
    if count == 0:
        print(f"[!] Structure lost during scan at Layer {i}. Stopping.")
        break
        
    # --- Tracking (Keep the camera centered on the chaos as it grows) ---
    # Use a high tracking blend for stability at extreme depths
    ...
    if i % 10 == 0:
        print(f"    [REC] Layer {i}: Zoom={current_zoom:.4e} | Pts={count} | Span={span:.4e}")
        
    # --- SCANNING DOWN (Zoom OUT) ---
    # Zoom out by a fixed exponential rate to map the growing structure.
