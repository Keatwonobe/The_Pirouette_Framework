...
def seek_absolute_tip_hyper_optimized(start_zoom, start_cx, start_cy, N_parts, 
    ...
    for i in range(max_iterations):
        ...
        # 4. UPDATE ZOOM (WITH RATCHET)
        target_zoom = span * 0.75
        
        # === FIX: THE RATCHET ===
        # Never allow the zoom to grow larger (e+...). Only shrink.
        # We take the MIN of the current zoom and the new target.
        # We also limit the shrink speed to 0.5x to prevent total collapse.
        potential_new_zoom = max(target_zoom, curr_zoom * 0.5)
        curr_zoom = min(curr_zoom, potential_new_zoom)
        # ========================
        
        # 5. Stop Condition (1e-60 limit)
        if curr_zoom < 1e-60:
            break
