...
class ParticleTracker:
    ...
    def update(self, pulse):
        ...
        if self.type == 'STRONG':
            damp = 0.5
        else: # GHOST sources (HETEROGENEOUS TRACKING)
            if self.id in [0, 1]: # G-1, G-2 (Stronger Ghosts)
                damp = 0.6  # Quick, responsive lock
            else: # self.id == 2 (G-3, Weakest Ghost)
                damp = 0.1  # Slow, highly stable lock
...
def render_spectrograph(trackers, width, height):
    ...
    # --- GLOBAL SPECTROGRAPH: Find GLOBAL Min/Max across all ghost trackers ---
    all_data = []
    for g in ghosts:
    ...
    for g in ghosts:
        ...
        for i, val in enumerate(data):
            x = ox + i * step_x
            # Normalize against the GLOBAL min/max
    ...
    # Draw reference line at 50% of the global max range
...
def run_ghost_hunt():
    ...
    for f in range(FRAMES):
        ...
        # Draw polygons using the current (tracked) positions
