...
# ----------------------------------------
    # PLOTTING 3D (Focusing on the Soliton Orbit)
    # ----------------------------------------
    ...
    # --- Subsample the data ---
    SUBSAMPLE_RATE = 10
    M = np.array(hist_m[::SUBSAMPLE_RATE])
    L = np.array(hist_lam[::SUBSAMPLE_RATE])
    P = np.array(hist_pm[::SUBSAMPLE_RATE])
    C = np.array(colors[::SUBSAMPLE_RATE]) 
    
    # --- MODIFIED PLOTTING LOGIC: Draw line segments ---
    # 1. Normalize the color array C (time index) to the range [0, 1]
    norm = plt.Normalize(C.min(), C.max())
    
    # 2. Get the 'hsv' colormap
    cmap = plt.get_cmap('hsv')
    
    # 3. Iterate over segments and plot them individually with their color
    # Note: We must iterate over all but the last point
    for i in range(len(M) - 1):
        # Get the color for the current segment
        color = cmap(norm(C[i]))
        
        # Plot the segment between point i and point i+1
        ax.plot(
            M[i:i+2], 
            L[i:i+2], 
            P[i:i+2], 
            color=color, # Pass the single calculated color
            linewidth=1.0, 
            alpha=0.9
        )
    ...
    # Title reflects the focus on the topological motion
    ax.set_title(f"Fermion Soliton Orbit (Twist = {TWIST})", color='white', fontsize=16)
    
    # Optimal View for the "Double Loop"
    ax.view_init(elev=20, azim=60)

    plt.tight_layout()
    plt.savefig('fermion_soliton_orbit.png')
    # plt.show() # Remove plt.show() when running in a notebook/headless environment
