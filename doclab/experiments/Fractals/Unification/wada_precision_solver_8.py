    ...
    for i in range(max_iterations):
        ...
        if i < 5 or i % 10 == 0:
            ...
            if curr_zoom < 1e-16 and abs(curr_cx_nd[1]) < 1e-30:
                 print("  [DEBUG] Tail check: High part only? (Potential precision loss)")

        # --- ZOOM STRATEGY: GRAVITY ASSIST ---
        # 1. Calculate the Target based on the structure size (standard autofocus)
        target_from_span = span * 0.6
        
        # 2. Calculate the "Forced Descent" limits
        # We MUST zoom in by at least 1.25x (multiplier 0.8) every frame.
        # We MUST NOT zoom in faster than 100x (multiplier 0.01) every frame.
        force_min_decay = curr_zoom * 0.8  # Slowest allowed zoom-in
        force_max_decay = curr_zoom * 0.01 # Fastest allowed zoom-in
        
        # 3. Apply the logic
        # If the span suggests we should stay at 1.0 (target > force_min), 
        # we ignore it and force the dive to force_min.
...
if __name__ == '__main__':
    ...
    print(f"[-] Starting AGGRESSIVE Deep Dive (N={N_PARTS}) from ({START_CX}, {START_CY})...")
