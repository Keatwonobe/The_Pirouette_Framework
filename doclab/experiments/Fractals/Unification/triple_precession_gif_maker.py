...
TOTAL_FRAMES = 200 # Increased frames for longer chaotic trace
PANEL_RES = 600
...
def render_map_frame(frame_index, histories, com_history):
    """
    ...
    center_m, center_l = com_history[frame_index]
...
def run_illuminator_frames(p1_history):
    ...
    (MODIFIED to return camera and peak history for calculation)
    """
    ...
    # Data storage for Winding Number Calculation
    camera_history = []
    peak_history = []
    ...
    for f in range(TOTAL_FRAMES):
        ...
        camera_history.append((solver_m, solver_l))
        peak_history.append((true_m, true_l))
        
        # Update camera (solver) position
    ...
    return frames_buffer, camera_history, peak_history

def calculate_winding_number(history_points, center_points):
    """Calculates the total angular displacement (winding number) in radians."""
    
    total_angle_change = 0.0
    
    for i in range(len(history_points) - 1):
        m_curr, l_curr = history_points[i]
        m_next, l_next = history_points[i+1]
        
        center_m, center_l = center_points[i]
        
        # Vector from center to current point
        dm_curr = m_curr - center_m
        dl_curr = l_curr - center_l
        
        # Vector from center to next point
        dm_next = m_next - center_m
        dl_next = l_next - center_l
        
        # Calculate the angle of the vectors
        angle_curr = np.arctan2(dl_curr, dm_curr)
        angle_next = np.arctan2(dl_next, dm_next)
        
        # Calculate the change in angle, correcting for the -pi to pi wrap-around
        delta_angle = angle_next - angle_curr
...
def combine_and_save():
    ...
    # 1. Run Simulation and get the full paths for all three quarks + COM
    simulator = ThreeQuarkSimulator()
    p1_history, p2_history, p3_history, com_history = simulator.simulate_all()
    
    # 2. Render Illuminator Frames (based on P1 path)
    illuminator_frames, camera_history, peak_history = run_illuminator_frames(p1_history)
    
    # --- CALCULATIONS ---
    print("--- 🧮 CALCULATING WINDING NUMBERS ---")

    # A. Quark Trajectory Winding (P1 around COM)
    # The center is the COM position at each frame
    quark_winding_rad = calculate_winding_number(p1_history, com_history)
    quark_winding_turns = quark_winding_rad / (2 * np.pi)
    print(f"P1 Quark Trajectory Winding (Turns): {quark_winding_turns:.3f}")
    
    # B. Helical Ribbon Winding (Peak around Camera Center)
    # The history points are the 'true_m', 'true_l' (peak) positions.
    # The center points are the 'solver_m', 'solver_l' (camera) positions.
    # This measures the rotation of the field intensity relative to the tracking camera.
    helicity_winding_rad = calculate_winding_number(peak_history, camera_history)
    helicity_winding_turns = helicity_winding_rad / (2 * np.pi)
    print(f"Illuminator Helicity Winding (Turns): {helicity_winding_turns:.3f}")
    ...
    for f in range(TOTAL_FRAMES):
        ...
        map_frame = render_map_frame(f, all_histories, com_history)
