...
# =========================================================
#  PIROUETTE: UNIFIED RENDERER (GEMINI V6 REFACTORED V2)
# =========================================================

# --- CONFIG ---
OUTPUT_FILENAME_COMBINED = "triple_precession_view_refactored_v2.gif"
TOTAL_FRAMES = 450
PANEL_RES = 600
...
def draw_precession_trail(draw_target, current_history, full_history_len, center_m, center_l, res_x, res_y, zoom, angle, trail_color, current_p_color, draw_line_gradient=False):
    ...
    draw.ellipse([cx-4, cy-4, cx+4, cy+4], fill=current_p_color) 


def render_map_frame(frame_index, histories):
    """
    Renders the wide-angle field centered on P1, but draws the trails of P1, P2, and P3.
    """
    
    P1_history, P2_history, P3_history = histories
    
    # Center the map on P1's position
    center_m, center_l = P1_history[frame_index]
    pulse = 1.0 + 0.15 * np.sin(frame_index * 0.1)

    # RENDER: WIDE-ANGLE STATIC VIEW, centered on the particle P1
    ...
    draw.text((10, 10), "MANIFOLD PRECESSION TRACE (P1-CENTERED)", fill=(255, 200, 100))
    draw.text((10, 30), f"Center P1 CoRot: ({center_m:.2f}, {center_l:.2f})", fill=(255, 200, 100))
...
def run_illuminator_frames(p1_history):
    ...
    (This function is unchanged from the previous refactor.)
    """
...
def combine_and_save():
    ...
    for f in range(TOTAL_FRAMES):
        ...
        # 3. Render the Wide Map Frame (Now drawing P1, P2, P3)
