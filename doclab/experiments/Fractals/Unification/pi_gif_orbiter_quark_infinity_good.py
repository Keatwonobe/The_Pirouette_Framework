...
def run_ghost_hunt():
    ...
    def angle_of_triangle(pts):
        # pts = [(m1,l1),(m2,l2),(m3,l3)]
        A = np.array(pts[0])
        B = np.array(pts[1])
        C = np.array(pts[2])
        v1 = B - A
    ...
    # compute twist constant across frames
    strong_hist = [t.history for t in trackers if t.type == "STRONG"]

    # unpack per-frame triangles
    angles = []
    for f in range(min(map(len,strong_hist))):
        tri = [strong_hist[0][f], strong_hist[1][f], strong_hist[2][f]]
        angles.append(angle_of_triangle(tri))

    angles = np.unwrap(np.array(angles))
    twist_per_frame = np.diff(angles)

    print("Mean twist per frame:", twist_per_frame.mean())
    print("Total twist:", angles[-1] - angles[0])
