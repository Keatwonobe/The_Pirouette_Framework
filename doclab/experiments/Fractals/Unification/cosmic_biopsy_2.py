...
K_REALITY = 1.0
K_TRAVELER = 0.9
SMOOTHING_SIGMA = 10  # Degrees of smoothing (Defocus to see "Massive Objects")
...
def main():
    ...
    # 4. Find the Peaks (Voids?) and Troughs (Mass?)
    ...
    # Convert index to Galactic Coords
    ...
    print("="*40)
    print(f"MAXIMUM HEALING (Most 'Relaxed' Region):")
    print(f"  Coords: (l={l_max:.1f}, b={b_max:.1f})")
    print(f"  Score:  {healing_smooth[max_idx]:.4f}")
    print("-" * 40)
    print(f"MAXIMUM TENSION (Most 'Stressed' Region):")
    ...
    # Mark the Peaks
    plt.plot(l_max if l_max <= 180 else l_max-360, b_max, 'g*', markersize=20, label='Max Healing (Void?)')
    plt.plot(l_min if l_min <= 180 else l_min-360, b_min, 'rX', markersize=20, label='Max Tension (Mass?)')
    
    # Mark Known Objects for reference
    ...
    plt.title(f"The 'Massive Object' Map (Smoothed sigma={SMOOTHING_SIGMA})\nWhere does the Universe Relax?")
    plt.xlabel("Galactic Longitude")
    plt.ylabel("Galactic Latitude")
    plt.legend(loc='lower right')
    
    plt.savefig("cmb_massive_object_scan.png")
    print("✅ Scan complete. Saved to cmb_massive_object_scan.png")
