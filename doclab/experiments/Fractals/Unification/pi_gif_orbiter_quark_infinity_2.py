...
def recursive_drill_down(target_layer):
    ...
    Returns a list of (r_lock, theta_lock, final_width) for each layer.
    """
    layer_locks = []
    ...
    for layer in range(1, target_layer + 1):
        ...
        layer_locks.append((lock_r, lock_theta, config["FINAL_ZOOM_WIDTH"]))
...
def generate_microscope_gif():
    ...
    # Total DIVE frames will be DIVE_FRAMES per layer transition
    zoom_layers = [1.5] + [lock[2] for lock in layer_locks] # Start at 1.5
    total_dive_frames = DIVE_FRAMES * N_LAYERS
    total_frames = total_dive_frames + ORBIT_FRAMES
    
    # Generate the smooth, overall zoom curve (Logarithmic descent)
    # We combine the log spaces for a smoother transition than sharp steps.
    combined_zoom_levels = np.logspace(np.log10(zoom_layers[0]), np.log10(zoom_layers[-1]), total_dive_frames)
    
    print(f"Rendering Combined Dive ({total_dive_frames} frames) & Orbit sequence ({ORBIT_FRAMES} frames)...")
    
    # --- RENDER LOOP ---
    output_filename = "proton_microscope_infinity_zoom.gif" # New single output file

    # Get the final lock coordinates for the orbit phase
    final_lock_r, final_lock_theta_init, final_width = layer_locks[-1]
    ...
    for f in range(total_frames):
        ...
        if f < total_dive_frames:
            # DIVE PHASE: Use the combined log-scale zoom
            current_width = combined_zoom_levels[f]
            
            # --- CAMERA PATH INTERPOLATION ---
            
            # We determine which layer transition we are currently in.
            # This is key for tracking the lock from layer-to-layer.
            
            # Interpolation factor (0 to 1) for the entire dive
            global_t = f / (total_dive_frames - 1) 
            
            # Linearly interpolate between the initial hint (0,0) and the final lock.
            # This provides a smooth path that tracks the target as it is found
            # by each successive layer's 'lock'.
            
            # The initial target for the camera is the Layer 1 hint.
            # We can use the final lock's r/theta as a smooth tracking guide.
            
            # Simple approach: Stick to the final lock, but let the rotation happen
            # The focus must always be on the final target for a continuous zoom.
        ...
        # Render
        # ... (Rendering and normalization code remains the same)
        ...
        if f % 10 == 0:
            print(f"  Frame {f}/{total_frames} | Zoom Width: {current_width:.12f} (Camera: {cam_m:.8f}, {cam_l:.8f})")
    ...
    print("✅ DONE. Generated proton_microscope_infinity_zoom.gif")
