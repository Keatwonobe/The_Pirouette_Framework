...
# Helper function to process a single row in parallel
def process_row(args):
    """
    Computes a row of the helicity grid.
    args = (i, lam, m_vals, res)
    Returns: (row index, numpy array of helicity values)
    """
    i, lam, m_vals, res = args
    row = np.zeros(res, dtype=float)
    for j, m in enumerate(m_vals):
        row[j] = measure_helicity(m, lam)
    return i, row
...
def compute_helicity_grid(res=RES_BASE):
    ...
    # Prepare arguments for multiprocessing: list of (row_index, lambda_value, m_values_array, resolution)
    pool_args = [(i, lam, m_vals, res) for i, lam in enumerate(l_vals)]

    # Use multiprocessing to run row calculations in parallel
    # Determine number of cores automatically for best efficiency
    num_cores = os.cpu_count() if os.cpu_count() is not None else 4
    print(f"Starting grid computation with {num_cores} parallel processes...")

    # The Pool manages the worker processes
    with multiprocessing.Pool(processes=num_cores) as pool:
        # map() is used to distribute the arguments (pool_args) to the worker function (process_row)
        # results is a list of (row_index, row_data) tuples
        results = pool.map(process_row, pool_args)

    # Reassemble the grid from the results
...
if __name__ == "__main__":
    # This check is crucial for multiprocessing to work correctly on all systems
    if __name__ == "__main__":
        # The main logic is wrapped inside this check
