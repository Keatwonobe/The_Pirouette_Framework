...
def generate_oracle_map_centered(res, zoom, center_x, center_y):
    ...
    for y in prange(res):
        for x in range(res):
            ...
            # 🌟 FIX 2: Skip calculation if coordinates are invalid (NaN or Inf)
...
MAX_LAYERS = 800        # How tall the tower is
...
print(f"[-] Starting HIGH RES Structural Scan (Upwards from Tip)...")
...
for i in range(MAX_LAYERS):
    ...
    cx = cx * 0.1 + new_cx * 0.9 # Snappier tracking for high zoom
    ...
    # We want the 'span' (size of object) to fill the screen, plus a small margin.
    # target_zoom is "half width of screen".
    # If span is 0.5, we want screen width to be roughly 0.6.
    # So target_zoom should be (span / 2) * margin_factor.
