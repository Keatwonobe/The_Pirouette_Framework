...
N_POINTS = 5000           # Resolution along the helical path
...
def get_ylm(m, l, phi, theta):
    """
    Wrapper to handle SciPy deprecation. 
    scipy < 1.15 uses sph_harm(m, l, phi, theta)
    scipy >= 1.15 uses sph_harm_y(l, m, phi, theta)
    """
    try:
        # Try new function first (SciPy 1.15+)
        # Note: sph_harm_y takes (l, m, phi, theta)
...
def synthesize_along_helix(alms, lmax, theta_path, phi_path, k_twist=1.0):
    ...
    for l in range(lmax + 1):
        for m in range(-l, l + 1):
            ...
            # Uses helper to check for sph_harm vs sph_harm_y
            Y_lm = get_ylm(m, l, phi_path, theta_path)
...
def main():
    ...
    for i, k in enumerate(K_SCAN):
        ...
        # Open with context manager and use copy()
        with Image.open(fname) as img:
            frames.append(img.copy()) 
            
        os.remove(fname) # Now safe to remove
