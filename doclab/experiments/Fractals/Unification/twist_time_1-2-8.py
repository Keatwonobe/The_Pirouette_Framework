...
def run_128_scan():
    ...
    print("Target fractions: G=%.4f, T=%.4f, R=%.4f"
    ...
    for twist in TWIST_VALUES:
        ...
        print(f"{twist:5.2f}  {g:7.4f}  {t:7.4f}  {r:7.4f}  {err:10.6f}")
    ...
    for k in range(10):
        i = idx_sorted[k]
        print(f"  #{k+1:2d}: TWIST={TWIST_VALUES[i]:.3f}  "
              f"<G>={mean_G[i]:.4f}  <T>={mean_T[i]:.4f}  <R>={mean_R[i]:.4f}  "
              f"err={errors[i]:.6e}")
