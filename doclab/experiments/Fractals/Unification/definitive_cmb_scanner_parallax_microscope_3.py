...
# MICROSCOPE GEOMETRY
FOV_DEG = 25.0          # Zoomed out slightly to see the "Triangles"
N_RES = 400             
L_MIN = 20              
L_MAX = 120             # Higher resolution to catch fine filaments
PARALLAX_STRENGTH = 0.25 

# ANIMATION
FRAMES = 60
K_START = 0.92          # Sweeping through the "Recession" zone
K_END = 1.08
...
def run_strain_scanner():
    ...
    for i, k in enumerate(k_vals):
        ...
        ghost_filament = np.power(ghost_filament, 0.6) 

        # PLOTTING
        fig = plt.figure(figsize=(10, 12))
        gs = gridspec.GridSpec(2, 1, height_ratios=[2, 1], hspace=0.05)
        
        # Top Panel: Manifold Strain
        ax1 = plt.subplot(gs[0])
        ax1.imshow(ghost_filament, origin='lower', cmap='inferno', vmin=0.1, vmax=0.9)
        ax1.set_title(f"MANIFOLD STRAIN TENSION | k={k:.3f}", fontsize=12, fontweight='bold', color='white', backgroundcolor='black')
        ax1.axis('off')
        ax1.contour(field_main, levels=10, colors='cyan', alpha=0.2, linewidths=0.5)

        # Bottom Panel: Vector Components
        ax2 = plt.subplot(gs[1])
        strain_y, strain_x = np.gradient(ghost_filament)
        
        # RGB Vector Map
        composite = np.zeros((*ghost_filament.shape, 3))
        composite[..., 0] = np.clip(np.abs(strain_x) * 4.0, 0, 1) # Red (Horizontal)
        composite[..., 2] = np.clip(np.abs(strain_y) * 4.0, 0, 1) # Blue (Vertical)
        # Green channel highlights "Knots" where both X and Y strain are high
        composite[..., 1] = np.clip((composite[..., 0] * composite[..., 2]) * 2.0, 0, 1)
        
        ax2.imshow(composite, origin='lower')
        ax2.set_title("DIRECTIONAL STRESS (Red=H, Blue=V, White=Knot)", fontsize=10, fontweight='bold')
        ax2.axis('off')
