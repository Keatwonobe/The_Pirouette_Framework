...
def compute_topology_fields(
    ...
    You can now override resolution + (m, λ) window.
    ...
    C : 2D array
        Spin winding number (turns in phase space).
    """
    ...
    m_min = topo.M_MIN if m_min is None else m_min
    m_max = topo.M_MAX if m_max is None else m_max
    l_min = topo.L_MIN if l_min is None else l_min
    l_max = topo.L_MAX if l_max is None else l_max

    print(f"[FTM] Computing topology at {RES}x{RES}, steps={STEPS}, "
          f"m∈[{m_min},{m_max}], λ∈[{l_min},{l_max}]")
...
def map_flux_tubes(
    ...
    """
    1. Compute chaos+spin fields on a chosen window.
    2. Extract ridge skeleton of chaos.
    3. Find connected components (flux tubes).
    4. Optionally decode each tube's SU(3) mixture (local mass Hessian).

    decode_su3=False -> super fast "does the skeleton match the big fractal?"
    """
    # --- zoomable topology ---
    ...
    for idx, comp in enumerate(comps):
        ...
        if decode_su3:
            ...
            weights, mean_w, amps = None, None, None
    ...
    # --- visualization: chaos background + tubes ---
    fig, ax = plt.subplots(figsize=(10, 10))
    im = ax.imshow(
        Zs,
        extent=[M.min(), M.max(), L.min(), L.max()],
        origin="lower",
        cmap="magma"
    )
    ...
    for tr in tube_results:
        ...
        if decode_su3 and tr["mean_w"] is not None:
            cT = tr["mean_w"]["T"]
            # red→magenta as T-weight increases
            col = (1 - cT, 0.2, cT)
        else:
            # skeleton-only: use bright yellow tubes
            col = (1.0, 1.0, 0.2)

        ax.plot(m_coords, l_coords, "-", linewidth=0.7, color=col, alpha=0.9)

    ax.set_xlabel("Mass field m")
    ax.set_ylabel("Coupling field λ")
    ax.set_title("Flux Tube Mapper: chaos ridges + SU(3) color mix")
    plt.tight_layout()
    plt.savefig("flux_tube_map_zoomed_2.png", dpi=200)
    plt.show()
...
if __name__ == "__main__":
    # Example: big zoom-out matching your multiverse maps
    # (tweak ranges / RES to taste vs. runtime)

    tubes = map_flux_tubes(
        min_points=40,
        percentile=93,       # higher = thinner, cleaner ridges
        res=800,             # 800x800 grid; go higher if your GPU/CPU can take it
        m_min=-600.0,
        m_max=600.0,
        l_min=-600.0,
        l_max=600.0,
        decode_su3=False,    # first pass: just check skeleton vs space fractal
    )
