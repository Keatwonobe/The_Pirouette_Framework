...
import obspy.io.mseed as mseed
# print any attribute that looks like a packer registry or packer class
cands = [n for n in dir(mseed) if 'PACK' in n.upper()]
print("obspy.io.mseed contains:", cands)

# if there's a singleton dict under a different name, you might find it e.g.
if hasattr(mseed, '_PACKER_MAP'):
    print("  → keys:", mseed._PACKER_MAP.keys())
elif hasattr(mseed, '_PACKERS'):
    print("  → keys:", mseed._PACKERS.keys())
else:
    print("  → no _PACKERS or _PACKER_MAP here; maybe try STEIM1/INT32 directly")
...
class SyntheticSeismogramGenerator:
    ...
    def validate_config(self):
        ...
        required_keys = [
            ...
            'noise_level', 'target_int_amplitude', 'taper_duration_s' # New key for tapering
        ]
    ...
    def generate_signal(self):
        ...
        # --- NEW: Apply a taper to the envelope to smooth the edges ---
        taper_duration = self.config['taper_duration_s']
        taper_samples = int(taper_duration * sr)
        
        # Ensure the taper is not longer than half the wavelet
        taper_samples = min(taper_samples, wavelet_len // 2)
        
        if taper_samples > 0:
            # A hanning window provides a smooth cosine-based taper
    ...
    def write_mseed(self, signal_data):
        ...
        scaling and Steim2 compression.

        Args:
            signal_data (np.ndarray): The signal to write.
        """
        stats = obspy.core.trace.Stats()
        stats.network = self.config['station_info']['network']
        stats.station = self.config['station_info']['station']
        stats.location = self.config['station_info']['location']
        stats.channel = self.config['station_info']['channel']
        stats.starttime = obspy.UTCDateTime(datetime.now(timezone.utc))
        stats.sampling_rate = self.config['sampling_rate_hz']
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # NEW: Taper duration to smooth the start/end of each thump.
        # This is crucial for Steim compression. 0.05s is a good starting point.
    ...
    try:
        ...
        try:
            ...
            plt.title("Generated Synthetic 'Thumper' Signal (Tapered for Steim)")
            plt.xlabel("Time (s)")
