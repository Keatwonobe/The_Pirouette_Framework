"""
Ki Resonance Detector for HackRF One
Uses SoapySDR to interface with the HackRF and detect Ki-related resonance patterns
"""
import os
import sys
...
pothos_paths = [
    ...
    r"C:/Program Files/PothosSDR/lib",
    r"C:/Program Files/PothosSDR/lib/python3.9/site-packages"
]

for path in pothos_paths:
    if os.path.exists(path):
        os.environ["PATH"] = path + os.pathsep + os.environ["PATH"]
        if path not in sys.path and "site-packages" in path:
...
def setup_sdr(center_freq, sample_rate, gain=40):
    """Initialize the SDR device with the specified parameters"""
    # Find devices
    ...
    # If no HackRF, use the first available device
    if not hackrf_found:
        print("No HackRF found, using first available SDR device")
    ...
    print(f"SDR Device: Driver={driver}, Hardware={hardware}")
    
    # Configure device
    if driver.lower() == 'hackrf':
        # HackRF specific settings
        sdr.setGain(SOAPY_SDR_RX, 0, 'LNA', min(40, gain))
        sdr.setGain(SOAPY_SDR_RX, 0, 'VGA', min(62, gain))
    else:
        # Generic gain setting
        sdr.setGain(SOAPY_SDR_RX, 0, min(60, gain))
...
def capture_samples(sdr, duration_sec, chunk_size=65536):
    """Capture samples from the SDR for the specified duration"""
    ...
    for i in range(num_chunks):
        ...
        buffer_ptr = buffer.ctypes.data_as(SoapySDR._SoapySDR.c_void_p)
        
        # Read samples
        samples_to_read = min(chunk_size, num_samples - samples_collected)
        sr = sdr.readStream(rx_stream, [buffer_ptr], samples_to_read)
        
        if sr.ret > 0:
            # Copy to our samples array
            samples[samples_collected:samples_collected+sr.ret] = buffer[:sr.ret]
...
def plot_results(results, amplitude, phase, time, spectrum, freqs, center_freq, sample_rate, output_file=None):
    ...
    ax1.plot(time[:min(len(time), 10000)], amplitude[:min(len(amplitude), 10000)])  # Show first 10k samples for clarity
    ...
    phase_derivative = np.gradient(phase, time)
    ax2.plot(time[:min(len(time), 10000)], phase_derivative[:min(len(phase_derivative), 10000)])  # Show first 10k samples
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ki Resonance Detector for SDR Devices")
    ...
    try:
        ...
        sdr = setup_sdr(center_freq, sample_rate, gain)
        ...
        for session in range(1, args.sessions + 1):
            ...
            try:
                # Capture samples
                samples = capture_samples(sdr, duration)
        ...
        try:
            if 'sdr' in locals():
                # No explicit close method needed for SoapySDR
                print("SDR device released.")
        except:
            pass
