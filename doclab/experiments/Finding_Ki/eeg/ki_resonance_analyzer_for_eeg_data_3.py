...
def analyze_final_results(raw_csv_file, final_report_file):
    ...
    on a PER-CHANNEL basis, and save the final report.
    """
    ...
    print("Calculating Z-Scores on a per-channel basis...")

    # --- NEW: Per-Channel Z-Score Calculation ---
    # This is the key enhancement. We use groupby() to treat each channel separately.
    for metric in metrics_to_analyze:
        zscore_col = f'zscore_{metric}'
        # The transform() function applies the z-score calculation to each group (each channel)
        # and returns a Series with the same index as the original DataFrame.
        df[zscore_col] = df.groupby('channel')[metric].transform(
            lambda x: (x - x.mean()) / x.std() if x.std() > 0 else 0
        )
        print(f"  - Z-scores calculated for metric: '{metric}'")
    ...
    for metric in metrics_to_analyze:
        ...
        # Increased tolerance slightly as per-channel variance might be smaller
        rest_matches = df[np.abs(df[zscore_col] - KI_REST) < 0.15].sort_values(zscore_col, ascending=False)
        motion_matches = df[np.abs(df[zscore_col] - KI_MOTION) < 0.15].sort_values(zscore_col, ascending=False)
