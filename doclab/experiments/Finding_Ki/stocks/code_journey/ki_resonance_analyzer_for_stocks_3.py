...
def generate_raw_data(data_directory, raw_csv_file, chk_file, batch_size=1000):
    ...
    if resume_idx == 0 and file_exists:
        print("Starting a fresh run. Deleting existing raw data and checkpoint files.")
    ...
    with tqdm(total=len(all_files), initial=resume_idx, desc="Step 1: Generating Raw Data") as pbar:
        for i, filepath in enumerate(files_to_process):
            ...
            try:
                ...
                # --- MODIFIED SIGNAL: Use absolute daily PERCENTAGE change (return) ---
                # .pct_change() calculates (value[t] - value[t-1]) / value[t-1]
                # This normalizes the data across all price scales.
                price_trend_signal = df['Close'].pct_change().abs().dropna().to_numpy()
                
                # Filter out non-finite values that can result from division by zero
                price_trend_signal = price_trend_signal[np.isfinite(price_trend_signal)]

                if len(price_trend_signal) < 50:
                    files_skipped_no_movement += 1
                    continue

                rates, distortions = get_rate_distortion(price_trend_signal)
    ...
    print(f"Skipped (insufficient price history, <50 days): {files_skipped_no_movement} files.")
